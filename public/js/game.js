...
const COMPANY_LIFECYCLE_DB = {
  ...
  'Lotus': {
    ...
    retireFateEn:'Acquired by IBM for $3.5B. The spreadsheet king crushed by Excel.',
  },
  'WordPerfect': {
    retireYear:1994, retireQ:3, retireType:'acquired',
    retireFate:  'Novell에 $1.4B 인수. 이후 Corel에 $124M 재매각 — 99% 가치 증발.',
    retireFateEn:'Acquired by Novell for $1.4B, then resold to Corel for $124M — 99% value destroyed.',
  },
  'Ashton-Tate': {
    retireYear:1991, retireQ:2, retireType:'acquired',
    retireFate:  'Borland에 $439M 인수. dBase IV 품질 재앙이 브랜드를 소멸시켰다.',
    retireFateEn:'Acquired by Borland for $439M. The dBase IV quality disaster killed the brand.',
  },
  ...
  'Netscape': {
    ...
    retireFateEn:'Post-AOL merger, crushed by free-bundled IE. Browser discontinued.',
  },
  'Novell': {
    retireYear:2011, retireQ:2, retireType:'acquired',
    retireFate:  'Attachmate에 $2.2B 매각. NetWare 사업 완전 소멸.',
    retireFateEn:'Sold to Attachmate for $2.2B. NetWare business fully dissolved.',
  },
  'Oracle': {
    ...
    declineMsgEn: '🔴 Oracle: Larry Ellison calls cloud "nonsense" — retreating from cloud market',
    comebackYear:2016, comebackQ:2,
    comebackMsg:   '🔴 Oracle Cloud(OCI) + Autonomous DB 출시 — 클라우드 시장 재진입 선언!',
    comebackMsgEn: '🔴 Oracle Cloud (OCI) + Autonomous DB launch — re-entering the cloud market!',
  ...
  'SAP': {
    ...
    declineMsgEn: '🏭 SAP: Cloud transition delayed — Salesforce & Workday eroding core markets',
    comebackYear:2020, comebackQ:1,
    comebackMsg:   '🏭 SAP RISE with SAP — 클라우드 전환 완성. AI 시대 재도약 선언!',
    comebackMsgEn: '🏭 SAP RISE with SAP — cloud transition complete. AI era comeback declared!',
  ...
  'VMware': {
    ...
    retireFateEn:'Acquired by Broadcom for $61B. Subscription price hikes trigger mass customer exodus.',
  },
  'Red Hat': {
    retireYear:2019, retireQ:3, retireType:'acquired',
    retireFate:  'IBM에 $34B 인수. 오픈소스 하이브리드 클라우드 전략 편입.',
    retireFateEn:'Acquired by IBM for $34B. Integrated into open-source hybrid cloud strategy.',
  },
  'Twitter': {
    declineYear:2019, declineQ:2,
    declineMsg:   '🐦 Twitter: 광고 매출 정체·사용자 성장 둔화 — 수익성 위기 본격화',
    declineMsgEn: '🐦 Twitter: Ad revenue stagnates, user growth slows — profitability crisis deepens',
    retireYear:2022, retireQ:4, retireType:'acquired',
    retireFate:  'Elon Musk에 $44B 인수. X로 리브랜딩, 광고주·직원 대거 이탈.',
    retireFateEn:'Acquired by Elon Musk for $44B. Rebranded to X; advertisers & staff flee.',
  },
  'BlackBerry': {
    declineYear:2008, declineQ:2,
    declineMsg:   '📱 BlackBerry: iPhone 출시 후 기업 스마트폰 시장 급격히 잠식 — 터치스크린 대응 실패',
    declineMsgEn: '📱 BlackBerry: iPhone launch rapidly erodes enterprise smartphone market — touchscreen pivot fails',
    retireYear:2016, retireQ:3, retireType:'obsolete',
    retireFate:  '스마트폰 하드웨어 사업 종료. 보안 소프트웨어 회사로 전환 시도.',
    retireFateEn:'Smartphone hardware discontinued. Attempts pivot to security software company.',
  },
  'Zendesk': {
    retireYear:2022, retireQ:3, retireType:'acquired',
    retireFate:  'KKR·Permira 컨소시엄에 $10.2B 인수. 상장 폐지.',
    retireFateEn:'Acquired by KKR & Permira consortium for $10.2B. Taken private.',
  },
  'Slack': {
    retireYear:2021, retireQ:4, retireType:'acquired',
    retireFate:  'Salesforce에 $27.7B 인수. 독립 브랜드 유지하며 Salesforce 생태계 편입.',
    retireFateEn:'Acquired by Salesforce for $27.7B. Maintains independent brand within Salesforce ecosystem.',
  },
  'Workday': {
    declineYear:2023, declineQ:2,
    declineMsg:   '🏢 Workday: AI 네이티브 HR 솔루션에 성장 둔화 — 구조조정 착수',
    declineMsgEn: '🏢 Workday: Growth slows vs. AI-native HR solutions — restructuring begins',
    comebackYear:2025, comebackQ:1,
    comebackMsg:   '🏢 Workday AI — 생성형 AI 통합으로 HR 플랫폼 재도약!',
    comebackMsgEn: '🏢 Workday AI — Generative AI integration revives the HR platform!',
  ...
  'Snowflake': {
    ...
    declineMsg:   '❄️ Snowflake: CEO 교체 + Databricks·Google BigQuery 경쟼 심화로 성장 둔화',
    declineMsgEn: '❄️ Snowflake: CEO change + intensifying Databricks & BigQuery competition slows growth',
    comebackYear:2026, comebackQ:1,
    comebackMsg:   '❄️ Snowflake Arctic LLM + AI Data Cloud — 데이터+AI 통합으로 반격!',
    comebackMsgEn: '❄️ Snowflake Arctic LLM + AI Data Cloud — striking back with data+AI integration!',
  ...
  'Mistral AI': {
    ...
    declineMsgEn: '🌬️ Mistral AI: Big-tech open-source assault hits independent startup limits — funding stalls',
  },
  'Cohere': {
    declineYear:2026, declineQ:1,
    declineMsg:   '🔗 Cohere: 기업 AI 시장 OpenAI·Anthropic 양강 구도에 포지셔닝 어려움',
    declineMsgEn: '🔗 Cohere: Struggles to position against OpenAI·Anthropic duopoly in enterprise AI',
  },
};
  ...
  1: {
    ...
    casualties: [
      { match: /WordPerfect|Lotus|dBase|Ashton/i,
        ...
        reasonEn: 'Word processor and spreadsheet specialists failed to adapt to GUI and the internet. WordPerfect delayed Windows support by a year, ceding the market to MS Word. Lotus was crushed by Excel and sold to IBM for $3.5B in 1995.' },
      { match: /Novell/i,
        ...
        reasonEn: 'Grew on offline LAN OS (NetWare) but was late to TCP/IP internet. The overpriced WordPerfect acquisition ($1.4B) strained finances, and Windows NT eroded enterprise network share until Novell collapsed.' },
    ],
    // 시대 전환의 역사적 배경
    context: [
    ...
    contextEn: [
      '🌐 Windows 95 launch, Netscape IPO, and Java\'s debut in 1995 ignited the internet revolution simultaneously.',
      '📉 Offline packaged-software firms faced a double shock: distribution channels (floppy → internet) and business models (perpetual license → subscription) changed at the same time.',
      '💡 As internet access became essential enterprise infrastructure, the seeds of web-based SaaS were sown.',
    ],
    // 새로운 경쟁자 등장 배경
    ...
    entrantContextEn: '🚀 The internet boom and dot-com investment frenzy summoned new challengers to the PC-era giants. Microsoft evolved for the internet; Oracle and SAP locked up enterprise DB and ERP. But internet-first firms like Netscape, Yahoo, and Sun would also define this era.',
    // 생존한 기업에 대한 코멘트
    survivorContext:   'PC 시대를 살아남아 인터넷 혁명에 성공적으로 적응한 기업은 배포 채널 전환과 인터넷 기반 제품 재설계에 일찍 투자했다는 공통점이 있다.',
    survivorContextEn: 'Companies that survived the PC era and successfully adapted to the internet revolution shared one trait: they invested early in shifting distribution channels and redesigning products around the internet.',
  },
  ...
  2: {
    casualties: [
      { match: /Netscape|Yahoo|Sun|Borland|BEA|Novell|Siebel/i,
        ...
        reasonEn: 'Could not survive the double wave of the dot-com bust (2000–2002) and the AWS cloud shock (2006+). Netscape died in 2003 to IE bundling; Yahoo slowly declined after losing the search war to Google; Sun Microsystems saw its stock fall 98% post-bubble and was absorbed by Oracle in 2010.' },
      { match: /Siebel/i,
        reason:   '온프레미스 CRM의 황제였지만 Salesforce의 SaaS 모델에 속절없이 무너졌다. "소프트웨어는 구입하지 않고 빌린다"는 패러다임 전환을 끝내 이해하지 못했다.',
        reasonEn: 'The king of on-premises CRM crumbled helplessly to Salesforce\'s SaaS model. Siebel never grasped the paradigm shift: software is rented, not bought.' },
    ],
    context: [
    ...
    contextEn: [
      '📱 The 2007 iPhone launch permanently changed how software is distributed and consumed. The App Store was born, and "mobile-first" became the default enterprise strategy.',
      '☁️ The three-way battle of AWS (2006), Azure (2010), and GCP (2011) shattered the infrastructure cost model of firms running their own servers. Those that didn\'t migrate to cloud lost cost competitiveness.',
      '📉 First-generation internet companies that over-expanded during the dot-com boom ran out of capital or failed to validate their business models and vanished one by one.',
    ],
    ...
    entrantContextEn: '☁️ The SaaS subscription model and smartphone ecosystem define this era. iPhone opened the App Store; AWS opened the cloud. New giants — Google, Apple, Salesforce, Adobe, Workday — are reshaping enterprise software. Massive venture capital inflows are spawning a wave of DevOps, HCM, and collaboration startups.',
    survivorContext:   '인터넷 시대의 강자 중 모바일·클라우드로 살아남은 기업들은 하나같이 "클라우드 퍼스트" 전환에 막대한 R&D를 투자하고, 기존 온프레미스 매출의 잠식을 두려워하지 않은 공통점이 있다.',
    survivorContextEn: 'Internet-era leaders that survived into the mobile-cloud era shared one common trait: they poured massive R&D into a "cloud-first" transformation and were unafraid to cannibalize their existing on-premises revenue.',
  },
  ...
  3: {
    casualties: [
      { match: /Workday|Atlassian|Zendesk|HubSpot|Twilio/i,
        ...
        reasonEn: 'AI copilots and automation began replacing HR, CRM, and CS workflows. SaaS products that stayed on existing features without differentiated AI quickly lost ground to AI-native challengers.' },
      { match: /Dropbox|Box/i,
        ...
        reasonEn: 'Cloud storage itself became a commodity. Without AI-powered collaboration, search, and automation, differentiation was impossible. Microsoft 365 Copilot entered as a direct competitor, making independent survival increasingly difficult.' },
    ],
    context: [
    ...
    contextEn: [
      '🤖 ChatGPT\'s launch (Nov 2022) was the fastest technology adoption in history — 1M users in 5 days, 100M in 2 months. Enterprise software UI, UX, and pricing were all called into question.',
      '⚡ GitHub Copilot (2022), Claude (2023), and Gemini (2023) multiplied developer productivity 2–10×, collapsing the "output per developer" paradigm. Team sizes and per-unit development costs came under simultaneous pressure.',
      '💰 AI-native firms like OpenAI ($86B), Anthropic ($18B), and Mistral began threatening traditional SaaS valuations. Incumbents that couldn\'t embed AI faced the double burden of margin compression and growth stagnation.',
    ],
    ...
    entrantContextEn: '🤖 Generative AI is transforming every stage of software development, operations, and sales. OpenAI, Anthropic, Microsoft Copilot, and Google Gemini are entering the market and threatening legacy SaaS players. AI-native startups are delivering the same functionality as traditional SaaS with 1/10th the headcount.',
    ...
    survivorContextEn: 'Cloud-era survivors that adapted to the AI revolution deeply embedded AI into their products via proprietary LLMs or key AI partnerships, successfully pivoting from feature providers to "AI agent platforms".',
  },
  ...
  4: {
    casualties: [
      { match: /OpenAI|Anthropic|Mistral/i,
        ...
        reasonEn: 'After the AGI inflection point, early AI firms specialised in specific modalities lost their edge to general-purpose AGI systems. The application layer, proprietary data, and trust networks became more valuable than the models themselves.' },
      { match: /Zoom|Slack|Teams/i,
        reason:   '협업 도구의 경계가 흐려졌다. AGI 에이전트가 회의 준비·요약·후속 조치를 자동화하면서 사람 간 "동기 협업" 시간 자체가 80% 이상 줄었다.',
        reasonEn: 'The boundaries of collaboration tools blurred. AGI agents automated meeting prep, summaries, and follow-ups, cutting synchronous human collaboration time by more than 80%.' },
    ],
    context: [
    ...
    contextEn: [
      '✨ Around 2036, AGI systems began surpassing top human experts in specific knowledge domains. Full automation of software development, data analysis, and customer service became reality.',
      '🔄 The business software paradigm flipped from "humans using tools" to "AI agents assisting humans." The concept of a traditional SaaS user interface is disappearing.',
      '🌐 AGI-powered autonomous agents handle most repetitive enterprise tasks; the value of the software industry has shifted from "number of features" to "trust, security, and governance".',
    ],
    ...
    entrantContextEn: '✨ The new power players of the post-AGI era are not "feature-selling SaaS" companies but those building "autonomous agent ecosystems." Existing tech giants are fighting to control the AGI infrastructure layer, while new startups are carving niches with AGI-native solutions in specific industry verticals.',
    ...
    survivorContextEn: 'AI-revolution survivors that endured into the post-AGI era did more than bolt on AI features — they redesigned entire business processes around AGI agents and positioned trust, regulatory compliance, and data sovereignty as their core value propositions.',
  },
};
...
function priceLabel(p) {
  ...
  if (p <= 0.75) return { text: en ? 'Ultra Low'  : '초저가',  color:'#06b6d4' };
  if (p <= 0.90) return { text: en ? 'Discount'   : '할인가',  color:'#10b981' };
  if (p <= 1.10) return { text: en ? 'Market'     : '시장가',  color:'var(--muted)' };
  if (p <= 1.30) return { text: en ? 'Premium'    : '프리미엄', color:'#f59e0b' };
  return                { text: en ? 'Ultra High' : '초고가',  color:'#ef4444' };
}
...
function processTechDebt() {
  ...
  if (activeCount > 0) {
    if (totalSeverity > 1.0) {
      pushNewsBI(
        `🕰️ 기술 부채 ${activeCount}건 진행 중 (심각): 품질·기술·브랜드 급격히 하락 중`,
        `🕰️ ${activeCount} tech debt(s) active (critical): quality, tech & brand dropping fast`,
        'bad'
      );
    } else if (totalSeverity > 0.3) {
      pushNewsBI(
        `⚠️ 기술 부채 ${activeCount}건: 품질·기술·브랜드 계속 저하 중`,
        `⚠️ ${activeCount} tech debt(s): quality, tech & brand continuing to erode`,
        'warn'
      );
    }
  }
...
function processTurn() {
  ...
  if (attrition >= 3) pushNewsBI(
    `😤 직원 이탈: ${attrition}명 퇴사 (사기 ${G.morale}/100)`,
    `😤 Staff attrition: ${attrition} departed (morale ${G.morale}/100)`,
    attrition >= 8 ? 'bad' : 'warn'
  );
  ...
  if (G.employees < minEmp) {
    ...
    if (deficit > 0.4)
      pushNewsBI(
        `🚨 극심한 인력 부족 (${G.employees}/${minEmp}명): 매출 ${Math.round(deficit*100)}% 손실 · 품질 급락`,
        `🚨 Critical understaffing (${G.employees}/${minEmp}): ${Math.round(deficit*100)}% revenue loss · quality plummeting`,
        'bad'
      );
    else if (deficit > 0.2)
      pushNewsBI(
        `⚠️ 심각한 인력 부족 (${G.employees}/${minEmp}명): 매출 ${Math.round(deficit*100)}% 손실`,
        `⚠️ Severe understaffing (${G.employees}/${minEmp}): ${Math.round(deficit*100)}% revenue loss`,
        'bad'
      );
    else
      pushNewsBI(
        `⚠️ 인력 부족 (${G.employees}/${minEmp}명): 매출 ${Math.round(deficit*100)}% 손실`,
        `⚠️ Understaffed (${G.employees}/${minEmp}): ${Math.round(deficit*100)}% revenue loss`,
        'warn'
      );
  }
  ...
  if (G.loans && G.loans.length) {
    ...
    if (G.loans.length === 0) pushNewsBI('✅ 대출 전액 상환 완료!', '✅ All loans fully repaid!', 'good');
  }
...
function buildNews(prevRev, costs) {
  ...
  if (G.profit > 0)  lines.push({ t: t('news.profit.tag'), msg: t('news.profit.pos', {amt: fmt(G.profit)}), type:'good' });
  else               lines.push({ t: t('news.loss.tag'),   msg: t('news.profit.neg', {amt: fmt(G.profit)}), type:'bad' });

  if (shareDelta > 0.001)  lines.push({ t: t('news.share.up.tag'), msg: t('news.share.up.msg', {delta: (shareDelta*100).toFixed(3)}), type:'good' });
  if (shareDelta < -0.001) lines.push({ t: t('news.share.dn.tag'), msg: t('news.share.dn.msg', {delta: (shareDelta*100).toFixed(3)}), type:'bad' });

  const dead = G.competitors.filter(c => !c.alive && c._justDied);
  dead.forEach(c => { c._justDied = false; lines.push({ t: t('news.bankrupt', {name: c.name}), msg: t('news.bankrupt.msg'), type:'info' }); });
...
/** Push bilingual news — picks ko or en based on current language. */
function pushNewsBI(ko, en, type='info') {
  const msg = (typeof LANG !== 'undefined' && LANG.current === 'en') ? en : ko;
  pushNews(msg, type);
}
...
function processParadigmReset(newEraIdx) {
  ...
  const applyReset = (entity, isPlayer) => {
    ...
    if (isPlayer) {
      ...
      const eraNameStr = tEraName(newEraIdx);
      pushNewsBI(
        `🌐 ${eraNameStr} 패러다임 전환! 기술 ${prevTech}→${entity.tech} / 품질 ${prevQual}→${entity.quality} ` +
        `(적응도 ${adaptPct}% — ${adaptPct >= 80 ? '선도 전환 성공' : adaptPct >= 50 ? '부분 전환' : '⚠ 미적응 — 재투자 시급'})`,
        `🌐 ${eraNameStr} paradigm shift! Tech ${prevTech}→${entity.tech} / Quality ${prevQual}→${entity.quality} ` +
        `(adaptation ${adaptPct}% — ${adaptPct >= 80 ? 'led transition successfully' : adaptPct >= 50 ? 'partial transition' : '⚠ failed to adapt — reinvest urgently'})`,
      ...
      pushNewsBI(
        ...
        `🔄 New paradigm transition — for 8 quarters R&D & marketing benchmarks ×1.5 (previous investment level now insufficient)`,
        'warn'
      );
    }
  };
  ...
  if (narr) {
    // 1. 시대 전환 역사적 배경
    const _narrEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    const _narrCtx = (_narrEn && narr.contextEn) ? narr.contextEn : narr.context;
    _narrCtx.forEach(line => pushNews(line, 'info'));

    // 2. 생존 경쟁사 서사
    if (survivors.length > 0 && narr.survivorContext) {
      pushNewsBI(
        `✅ 생존 기업 (${survivors.join(', ')}): ${narr.survivorContext}`,
        `✅ Survivors (${survivors.join(', ')}): ${narr.survivorContextEn || narr.survivorContext}`,
        'good'
      );
    }
      ...
      narr.casualties.forEach(cas => {
        ...
        if (matched.length > 0) {
          pushNewsBI(
            ...
            `⚠️ Crisis warning — ${matched.join('·')}: ${cas.reasonEn || cas.reason}`,
            'bad'
          );
        }
      });
...
function checkEraEntrants() {
  ...
  if (newNames.length > 0) {
    ...
    const eraLabel = tEraName(currentEraIdx);
    pushNewsBI(
      `⚡ ${eraLabel} 개막! 새로운 강자들이 시장에 진입합니다.`,
      `⚡ ${eraLabel} begins! New powerhouses are entering the market.`,
      'warn'
    );

    // 시대 전환으로 재편된 경쟁 구도 — 역사적 배경 서사
    if (narr && narr.entrantContext) {
      const _ecEn = typeof LANG !== 'undefined' && LANG.current === 'en';
      pushNews((_ecEn && narr.entrantContextEn) ? narr.entrantContextEn : narr.entrantContext, 'info');
    }

    // 새 진입 기업 개별 뉴스
    newNames.forEach(n => pushNewsBI(
      `📢 ${n.name} 등장: ${n.msg}`,
      `📢 ${n.name} enters the market: ${n.msg}`,
      'info'
    ));
  }
  ...
  G.competitors.forEach(c => {
    ...
    if (lc.retireYear && !c._retired && c.alive && afterOrAt(lc.retireYear, lc.retireQ)) {
      ...
      const icon = lc.retireType === 'acquired' ? '💼'
                 ...
                 :                                  '📭';
      pushNewsBI(
        `${icon} ${c.name} 퇴장: ${lc.retireFate}`,
        `${icon} ${c.name} exits: ${lc.retireFateEn || lc.retireFate}`,
        'info'
      );
    }
    ...
    if (lc.declineYear && !c._declining && !c._retired && c.alive &&
      ...
      const _lcEn = typeof LANG !== 'undefined' && LANG.current === 'en';
      pushNews(_lcEn ? (lc.declineMsgEn || lc.declineMsg) : lc.declineMsg, 'warn');
    }
    ...
    if (lc.comebackYear && c._declining && !c._comeback && !c._retired && c.alive &&
      ...
      const _lcEn2 = typeof LANG !== 'undefined' && LANG.current === 'en';
      pushNews(_lcEn2 ? (lc.comebackMsgEn || lc.comebackMsg) : lc.comebackMsg, 'info');
    }
  });
}
  ...
  G.competitors.forEach(c => {
    ...
    if (isGiant && c.revenue > 0) {
      ...
      if (c.cash < giantFloor) {
        ...
        if (!c._raisedNews) {
          c._raisedNews = true;
          pushNewsBI(
            `🏦 ${c.name}: 시장 조달(채권·주식) ${fmt(raise)} — 경쟁력 유지`,
            `🏦 ${c.name}: raised ${fmt(raise)} via bonds/equity — maintaining competitiveness`,
            'info'
          );
        }
      } else {
    ...
    if (!c.ipo && c.alive && c.revenue >= 5000000 && c.profit > 0) {
      ...
      if (c._profitableQ >= 2) {
        ...
        pushNewsBI(`📈 ${c.name} IPO 상장! 시가총액 ${fmt(cap)}`, `📈 ${c.name} IPO! Market cap ${fmt(cap)}`, 'info');
      }
...
function renderHeader() {
  ...
  document.getElementById('hd-era').textContent     = tEraName(getEraIndex());
  ...
  const _hdEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('hd-emp').textContent     = G.employees.toLocaleString() + (_hdEn ? '' : '명');
  ...
  if (mbCash) {
    ...
    document.getElementById('mb-emp').textContent   = G.employees.toLocaleString() + (_hdEn ? '' : '명');
  }
}
...
function renderEraBanner() {
  ...
  document.getElementById('era-name').textContent = tEraName(getEraIndex());
...
function renderCostPreview() {
  ...
  const _cpBadgeEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const premBadge = investBonus > 0
    ? ` <span style="font-size:10px;color:#a78bfa;font-weight:400">+${Math.round(investPrem*100)}%p ${_cpBadgeEn ? 'inv.bonus' : '투자보너스'}</span>`
    : '';
  ...
  if (G.employees < minEmpProj) {
    ...
    const _swEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    staffWarnEl.innerHTML = _swEn
      ? `<div class="cpfw-label">🚨 Understaffed — ${Math.round(deficit*100)}% revenue loss projected</div>` +
        `<div class="cpfw-amount">Lost: <strong>${fmt(revLost)}</strong> · ` +
        `current ${G.employees} / min ${minEmpProj} required</div>`
  ...
  if ((G.eraTransitionQ || 0) > 0) {
    ...
    const _cpEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    eraTransEl.innerHTML = _cpEn
      ? `<div class="cpfw-label">🔄 Paradigm transition — R&D & marketing benchmark ×1.5 active</div>` +
        `<div class="cpfw-amount">Remaining: <strong>${G.eraTransitionQ} qtrs</strong> · prior investment level insufficient → stats decline</div>`
  ...
  if (projCash < 0) {
    ...
    const _fwEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    fundWarnEl.innerHTML = _fwEn
      ? `<div class="cpfw-label">⚠ Projected cash shortfall at quarter end</div>
         <div class="cpfw-amount">Shortfall: <strong>${fmt(shortfall)}</strong>
           (have: ${fmt(G.cash)} → projected: ${fmt(projCash)})</div>
         <button class="cpfw-btn" onclick="openFundingModal()">
           💰 Raise Capital (Loan · IPO)
         </button>`
...
function renderDashboard() {
  ...
  // ── 재무상태 ──
  const _dsEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  ...
  setFS('fs-loan',    loanBal > 0 ? fmt(loanBal) : (_dsEn ? 'None' : '없음'));
  setFS('fs-netcash', fmt(netCash));      setFSColor('fs-netcash', netCash);
  setFS('fs-emp',     G.employees.toLocaleString() + (_dsEn ? '' : '명'));
  ...
  const moraleLabel = G.morale >= 70 ? (_dsEn ? 'High 😊' : '높음 😊') : G.morale >= 40 ? (_dsEn ? 'Normal 😐' : '보통 😐') : (_dsEn ? 'Low 😟' : '낮음 😟');
  document.getElementById('k-morale').textContent = G.morale + '/100 ' + moraleLabel;
  setBar('k-morale-bar', G.morale, moraleColor);
  document.getElementById('k-attrition').textContent = G.lastAttrition + (_dsEn ? '/qtr' : '명/분기');
  const minEmp = calcMinEmployees();
  ...
  if (minEmpEl) {
    ...
    minEmpEl.textContent = _dsEn ? (minEmp + ' required') : (minEmp + '명 필요');
  ...
  if (tdPanel) {
    ...
    if (activeDebts.length === 0) {
      ...
      const tdCountLabel = _dsEn ? `${activeDebts.length} item(s)` : `${activeDebts.length}건`;
      tdPanel.innerHTML = `
        <div class="td-header">🕰️ ${_dsEn ? 'Tech Debt' : '기술 부채'} <span class="td-count">${tdCountLabel}</span></div>` +
        activeDebts.map(d => {
          ...
          const debtMeta  = _dsEn
            ? `${d.quartDebt} qtrs elapsed · Catch-up cost: ${fmt(cost)} (${Math.round(catchFrac * 100)}% of qtr revenue)`
            : `${d.quartDebt}분기 경과 · 현재 추격비용: ${fmt(cost)} (분기매출의 ${Math.round(catchFrac * 100)}%)`;
          return `
          <div class="td-item ${urgency}">
            <div class="td-title">${d.title}</div>
            <div class="td-meta">${debtMeta}</div>
          </div>`;
        }).join('');
    }
  }
  ...
  nl.innerHTML = items || `<div class="news-item" style="color:var(--muted)">${_dsEn ? 'No updates yet.' : '아직 동향이 없습니다.'}</div>`;
...
function renderMarketTab() {
  ...
  const _mkEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('mkt-size').textContent     = fmt(mktSize) + (_mkEn ? '/yr' : '/년');
  document.getElementById('mkt-my-share').textContent = myRelShare.toFixed(1) + '%';
  document.getElementById('mkt-era').textContent      = tEraName(getEraIndex());
  ...
  const pieBar = `
    <div class="market-pie-wrap">
      <div class="market-pie-label">${_mkEn ? `Peer market share (total 100%) — total market penetration: ${fmtPct(totalNamedShare,2)}` : `경쟁사 간 점유율 (합계 100%) — 전체 시장 침투율: ${fmtPct(totalNamedShare,2)}`}</div>
  ...
  const paradigmBadge = (tech) => {
    ...
    if (ratio >= 1.0)  return `<span class="para-badge para-ok">${_mkEn ? '✅ Adapted' : '✅ 적응'}</span>`;
    if (ratio >= 0.6)  return `<span class="para-badge para-mid">${_mkEn ? '⚡ Transitioning' : '⚡ 전환중'}</span>`;
    return                     `<span class="para-badge para-bad">${_mkEn ? '🔴 Not adapted' : '🔴 미적응'}</span>`;
  };
  ...
  const tableRows = all.map(c => {
    ...
    const statusBadge = !c.alive
      ? `<span class="ct-badge dead">${_mkEn ? '💀 Bankrupt' : '💀파산'}</span>`
      : c.cash < 0
        ? `<span class="ct-badge warn">${_mkEn ? '⚠ Loss' : '⚠적자'}</span>`
        : c.ipo
          ? `<span class="ct-badge ipo">${_mkEn ? '📈 Public' : '📈상장'}</span>`
          : '';

    const acqCell = !c.isPlayer && c.alive
      ? `<button class="ct-acq-btn ${canAfford?'':'disabled'}"
           onclick="${canAfford?`openAcquisitionModal(${c._idx})`:''}"
           title="${canAfford?(_mkEn?'Acquire':'인수 제안'):(_mkEn?'Insufficient cash '+fmt(acqPrice):'자금 부족 '+fmt(acqPrice))}">
           🤝 ${fmt(acqPrice)}
         </button>`
      : `<span style="color:var(--muted);font-size:10px">${c.isPlayer?(_mkEn?'(HQ)':'(본사)'):'—'}</span>`;

    // 실제 기업 역사 데이터 미니 표시
    const histLine = (() => {
      if (c.isPlayer || !c._hist || !c._hist.yr) return '';
      const h = c._hist;
      const revStr = h.rev >= 1000 ? `$${(h.rev/1000).toFixed(1)}B` : h.rev > 0 ? `$${h.rev}M` : (_mkEn?'N/A':'비공개');
      const mgStr  = h.opMg !== null ? `${Math.round(h.opMg*100)}%` : (_mkEn?'N/A':'비공개');
      const rpeStr = h.rpe  ? `$${Math.round(h.rpe/1000)}K${_mkEn?'/emp':'/인'}` : '';
      return _mkEn
        ? `<div class="ct-hist-ref" title="${h.note||''}">${h.yr} rev ${revStr} · op.margin ${mgStr}${rpeStr?' · '+rpeStr:''}</div>`
        : `<div class="ct-hist-ref" title="${h.note||''}">${h.yr}년 매출 ${revStr} · 영업이익 ${mgStr}${rpeStr?' · '+rpeStr:''}</div>`;
    })();
    ...
    <tr class="ct-row${c.isPlayer?' ct-player':''}${!c.alive?' ct-dead':''}">
      ...
      <td>
        ${c.alive ? (c.employees||0).toLocaleString()+(_mkEn?'':' 명') : '—'}
        ...
        ${c.alive ? (() => {
          ...
          return `<br><span style="font-size:10px;color:${mColor}">${mIcon} ${_mkEn?'Morale':'사기'} ${Math.round(m)}</span>`;
        })() : ''}
      </td>
      ...
      <td>${c.alive ? (() => {
        ...
        return `<span style="font-size:10px;color:var(--muted)">${_mkEn?'Wage':'임금'}</span><br>
           <span style="font-size:11px;color:${wColor}">${fmt(cSal)}${_mkEn?'/yr':'/년'} ${wLabel}</span><br>
           <span style="font-size:10px;color:var(--muted)">${_mkEn?'Productivity':'생산성'}</span><br>
           <span style="font-size:11px;color:${prodColor}">${fmt(revPerEmp)}${_mkEn?'/emp':'/인'}</span>`;
      })() : '—'}</td>
      <td>${(() => {
        if (!c.alive) return '—';
  ...
  <div class="ct-wrap">
    <table class="comp-table-grid">
      <thead>
        <tr>
          <th>${_mkEn?'Company':'기업'}</th>
          <th>${_mkEn?'Share':'점유율'}</th>
          <th>${_mkEn?'Qtr Revenue':'분기 매출'}</th>
          <th>${_mkEn?'Net Income':'순이익'}</th>
          <th>${_mkEn?'Cash':'현금'}</th>
          <th>${_mkEn?'Valuation':'기업가치'}</th>
          <th>${_mkEn?'Staff/Morale':'직원/사기'}</th>
          <th>${_mkEn?'Quality':'품질'}</th>
          <th>${_mkEn?'Brand':'브랜드'}</th>
          <th>${_mkEn?'Tech':'기술'}</th>
          <th>${_mkEn?'Wage/Productivity':'임금/생산성'}</th>
          <th>${_mkEn?'Price':'가격'}</th>
          <th>${_mkEn?'Acquire':'인수'}</th>
        </tr>
      </thead>
...
function styleLabel(s) {
  const en = typeof LANG !== 'undefined' && LANG.current === 'en';
  const m = en
    ? { differentiation:'Differentiation', cost_leadership:'Cost Leadership', focus:'Focus',
        aggressive:'Aggressive Growth', quality:'Quality First', value:'Cost Leadership', niche:'Niche Focus' }
    : { differentiation:'차별화', cost_leadership:'원가우위', focus:'집중화',
...
function renderTheoryTab() {
  const _thEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  // BCG Matrix
  ...
  let pos = '', posColor = '';
  if (isHighGrowth  && isHighShare)  { pos = _thEn ? '⭐ Star — Focus investment!'         : '⭐ Star — 집중 투자!';      posColor='#f59e0b'; }
  if (!isHighGrowth && isHighShare)  { pos = _thEn ? '🐄 Cash Cow — Harvest profits'       : '🐄 Cash Cow — 수익 창출';   posColor='#10b981'; }
  if (isHighGrowth  && !isHighShare) { pos = _thEn ? '❓ Question Mark — Investment call'   : '❓ Question Mark — 투자 결정'; posColor='#8b5cf6'; }
  if (!isHighGrowth && !isHighShare) { pos = _thEn ? '🐕 Dog — Reconsider strategy'        : '🐕 Dog — 재검토 필요';      posColor='#ef4444'; }
  ...
  document.getElementById('bcg-display').innerHTML = `
    ...
    <div style="font-size:10px;color:var(--muted);margin-top:3px">
      ${_thEn ? 'Market growth' : '시장 성장률'}: ${(mktGrowth*100).toFixed(1)}% | ${_thEn ? 'Share' : '점유율'}: ${fmtPct(G.marketShare,3)}
    </div>`;

  const bcgCells = _thEn
    ? [
        {name:'⭐ Star',     label:'High growth · High share', active:isHighGrowth&&isHighShare},
        {name:'❓ Q.Mark',   label:'High growth · Low share',  active:isHighGrowth&&!isHighShare},
        {name:'🐄 Cash Cow', label:'Low growth · High share',  active:!isHighGrowth&&isHighShare},
        {name:'🐕 Dog',      label:'Low growth · Low share',   active:!isHighGrowth&&!isHighShare},
      ]
    : [
  ...
  document.getElementById('plc-display').innerHTML = `
    ...
    <div style="font-size:10px;color:var(--muted);margin-top:6px">
      ${_thEn ? 'Current stage' : '현재 단계'}: <strong style="color:${stageColors[curIdx]}">${stageNames[curIdx]}</strong>
      (${G.productStageQ}${_thEn ? ' qtrs' : '분기째'})
    </div>`;
  ...
  document.getElementById('blue-ocean-val').textContent =
    boScore.toFixed(0) + '/100 — ' + (boScore > 60
      ? (_thEn ? 'Blue Ocean pioneering' : '블루오션 개척 중')
      : (_thEn ? 'Red Ocean competition' : '레드오션 경쟁 중'));
}
...
function initCharts() {
  ...
  const _chEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  charts.rev = makeChart('chart-rev', {
    type:'line',
    data:{ labels:[], datasets:[
      { label: _chEn ? 'Revenue'    : '매출',   data:[], borderColor:'#3b82f6', backgroundColor:'rgba(59,130,246,.1)', fill:true, tension:.3 },
      { label: _chEn ? 'Net Income' : '순이익', data:[], borderColor:'#10b981', backgroundColor:'rgba(16,185,129,.1)', fill:true, tension:.3 },
    ]},
  ...
  charts.tam = makeChart('chart-tam', {
    type:'line',
    data:{ labels:[], datasets:[
      { label: _chEn ? 'Past/Present' : '과거/현재', data:[], borderColor:'#06b6d4', backgroundColor:'rgba(6,182,212,.12)', fill:true, tension:.4, borderWidth:2 },
      { label: _chEn ? 'Forecast'     : '예측',      data:[], borderColor:'#8b5cf6', backgroundColor:'rgba(139,92,246,.06)', fill:true, tension:.4, borderWidth:2, borderDash:[5,4] },
    ]},
    options:{ responsive:true, plugins:{legend:{position:'top'}},
              scales:{y:{ticks:{callback:v=>fmt(v)},grid:{color:'rgba(255,255,255,.04)'}},
                      x:{grid:{color:'rgba(255,255,255,.04)'}}} }
  ...
  charts.cash = makeChart('chart-cash', {
    type:'line',
    data:{ labels:[], datasets:[
      { label: _chEn ? 'Cash' : '현금', data:[], borderColor:'#f59e0b', backgroundColor:'rgba(245,158,11,.1)', fill:true, tension:.3 }
    ]},
  ...
  charts.org = makeChart('chart-org', {
    type:'bar',
    data:{ labels:[], datasets:[
      { label: _chEn ? 'Headcount' : '직원 수', data:[], backgroundColor:'rgba(139,92,246,.5)', yAxisID:'y1' },
      { label: _chEn ? 'Quality'   : '품질',    data:[], type:'line', borderColor:'#10b981', tension:.3, yAxisID:'y2' },
      { label: _chEn ? 'Morale'    : '사기',    data:[], type:'line', borderColor:'#f59e0b', tension:.3, yAxisID:'y2', borderDash:[4,3] },
    ]},
    options:{ responsive:true, plugins:{legend:{position:'top'}},
              scales:{
...
function updateCharts() {
  ...
  if (charts.share) {
    ...
    // 경쟼사 간 상대 점유율 (합계 100%) 기준 — 시장현황과 동일한 지표
    const _ucEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    const ds = [{ label: G.name + (_ucEn ? ' (vs peers)' : ' (경쟼사 간)'),
...
const FUNDING_OPTS = [
  {
    ...
    dilution: 8,
    en: { label: '🌱 Seed Investment (Angel)', badgeLabel: 'VC',
          desc: 'Attract angel investors. Secure initial growth capital.',
          condMsg: 'Within 3 years of founding · one-time only' },
  ...
  },
  {
    ...
    dilution: 18,
    en: { label: '🅐 Series A',  badgeLabel: 'VC',
          desc: 'Series A venture capital. Repeat eligible after 8-quarter cooldown.',
          condMsg: 'Qtr revenue $200K+ · equity >5% · 2 yrs since last raise' },
  ...
  },
  {
    ...
    dilution: 15,
    en: { label: '🅑 Series B',  badgeLabel: 'VC',
          desc: 'Large VC round. Enables global expansion and aggressive hiring. Repeatable.',
          condMsg: 'Qtr revenue $2M+ · equity >5% · 2 yrs since last raise' },
    cond: g => g.history.length
  ...
  },
  {
    ...
    dilution: 12,
    en: { label: '🅒 Series C',  badgeLabel: 'VC',
          desc: 'Late-stage growth round. Repeatable.',
          condMsg: 'Qtr revenue $20M+ · equity >5% · 2 yrs since last raise' },
    cond: g => g.history.length
  ...
  },
  {
    ...
    dilution: 20,
    en: { label: '🏛 IPO (Public Offering)', badgeLabel: 'IPO',
          desc: 'Stock market listing. Massive public capital raise. Public company obligations apply.',
          condMsg: 'Qtr revenue $100M+ & equity >30% · one-time only' },
  ...
  },
  {
    ...
    en: { label: '🏦 Small Business Loan', badgeLabel: 'Loan',
          desc: 'Bank SME loan. 8-quarter (2-year) repayment. Limit & rate vary by credit rating.',
          condMsg: 'Max 2 small business loans at once' },
  ...
  },
  {
    ...
    en: { label: '🏦 Mid-Size Business Loan', badgeLabel: 'Loan',
          desc: 'Bank mid-size loan. 12-quarter (3-year) repayment. Limit & rate vary by credit rating.',
          condMsg: 'Qtr revenue $1M+ · no duplicate of same loan' },
  ...
  },
  {
    ...
    en: { label: '🏦 Large Syndicate Loan', badgeLabel: 'Loan',
          desc: 'Large syndicated loan. 16-quarter (4-year) repayment. Limit & rate vary by credit rating.',
          condMsg: 'Qtr revenue $50M+ · no duplicate of same loan' },
  ...
  },
  {
    ...
    en: { label: '🚨 Emergency Bridge Loan', badgeLabel: 'Emergency',
          desc: 'Unconditional short-term emergency loan. 28% annual rate · 6-quarter repayment. Last resort before bankruptcy.',
          condMsg: 'Max 3 emergency loans at once' },
...
function openFundingModal() {
  const _fmEn    = typeof LANG !== 'undefined' && LANG.current === 'en';
  ...
  document.getElementById('fm-loans').textContent  = totalLoan > 0
    ? fmt(totalLoan) + (_fmEn ? ' remaining' : ' 잔여')
    : (_fmEn ? 'None' : '없음');
  ...
  if (fmCrEl) {
    fmCrEl.innerHTML = _fmEn
      ? `Credit rating <strong style="color:${crp.color}">${cr.grade}</strong>` +
        ` <span style="opacity:.65">(${crp.label} · score ${cr.score})</span>` +
        ` &nbsp;·&nbsp; Loan rate <strong>${(crp.rate*100).toFixed(1)}%</strong>` +
        ` &nbsp;·&nbsp; Limit mult <strong>×${crp.loanMult}</strong>`
  ...
  const renderOpt = opt => {
    const ok = opt.cond(G);
    const _optL   = (_fmEn && opt.en) ? opt.en.label      : opt.label;
    const _optBL  = (_fmEn && opt.en) ? opt.en.badgeLabel : opt.badgeLabel;
    const _optD   = (_fmEn && opt.en) ? opt.en.desc       : opt.desc;
    const _optCM  = (_fmEn && opt.en) ? opt.en.condMsg    : opt.condMsg;
    ...
    if (opt.badge === 'vc' && opt.id !== 'seed' && opt.id !== 'ipo') {
      ...
      if (remaining > 0)
        cooldownNote = _fmEn
          ? `<div class="fo-cond" style="color:var(--amber)">⏳ Re-raise available in ${remaining} qtrs</div>`
          : `<div class="fo-cond" style="color:var(--amber)">⏳ 재유치 가능까지 ${remaining}분기</div>`;
      else if (lastQ !== -Infinity)
        cooldownNote = _fmEn
          ? `<div class="fo-cond" style="color:var(--green)">✅ Re-raise available (last raise: ${elapsed} qtrs ago)</div>`
          : `<div class="fo-cond" style="color:var(--green)">✅ 재유치 가능 (이전 유치: ${elapsed}분기 전)</div>`;
    }

    const crNote  = (isLoan && !isEmergency)
      ? (_fmEn ? ` (credit ${cr.grade})` : ` (신용등급 ${cr.grade})`) : '';
    const detailText = opt.dilution
      ? (_fmEn
          ? `+${fmt(scaledAmt)} cash · ${effDil.toFixed(0)}% equity dilution`
          : `+${fmt(scaledAmt)} 현금 · 지분 ${effDil.toFixed(0)}% 희석`)
      : (_fmEn
          ? `+${fmt(scaledAmt)} · ${opt.quarters}-qtr repayment · ${(effRate*100).toFixed(1)}% p.a.${crNote}`
          : `+${fmt(scaledAmt)} · ${opt.quarters}분기 상환 · 연 ${(effRate*100).toFixed(1)}%${crNote}`);
    const eraTag = eraMult > 1.5
      ? ` <span style="font-size:9px;color:var(--muted)">${_fmEn ? `(${G.year} scale)` : `(${G.year}년 기준)`}</span>` : '';
    const emStyle = isEmergency ? ' style="border-color:var(--red);background:rgba(239,68,68,0.06)"' : '';
    const amtLabel = opt.dilution ? (_fmEn ? 'raised' : '조달') : (_fmEn ? 'loan' : '대출');

    return `<div class="fund-option ${ok?'':'disabled'}"${emStyle} ${ok?`onclick="applyFunding('${opt.id}')"`:''}>
      <div class="fo-header">
        <span class="fo-title">${_optL}</span>
        <span class="fo-badge ${opt.badge}">${_optBL}</span>
      </div>
      <div class="fo-amount">${fmt(scaledAmt)} ${amtLabel}${eraTag}</div>
      <div class="fo-detail">${detailText}</div>
      <div class="fo-detail">${_optD}</div>
      ${cooldownNote}
      ${!ok ? `<div class="fo-cond">🔒 ${_optCM}</div>` : ''}
    </div>`;
  };
  ...
  const crisisHeader = isCrisis
    ? `<div style="background:rgba(239,68,68,0.12);border:1px solid var(--red);border-radius:6px;
         ...
         🚨 ${(typeof LANG!=='undefined'&&LANG.current==='en') ? 'Insufficient cash — immediate funding required' : '현금 부족 — 즉시 자금 조달이 필요합니다'}
       </div>`
    : '';
  ...
  const lockedSection = locked.length
    ? `<div style="margin-top:8px;padding-top:8px;border-top:1px solid var(--line2);
         ...
         🔒 ${(typeof LANG!=='undefined'&&LANG.current==='en') ? 'Options requiring additional conditions' : '현재 조건 미충족 옵션'}
...
function applyFunding(id) {
  ...
  if (opt.dilution) {
    ...
    const _afEnLabel = (opt.en && opt.en.label) || opt.label;
    const dilLabel    = dm.vcDil !== 1 ? ` (난이도 보정 ${effectiveDil.toFixed(0)}%)` : '';
    const dilLabelEn  = dm.vcDil !== 1 ? ` (difficulty adj. ${effectiveDil.toFixed(0)}%)` : '';
    pushNewsBI(
      `💰 ${opt.label} 완료! +${fmt(scaledAmt)} 현금 유치 · 지분 ${G.equity.toFixed(1)}% 보유${dilLabel}`,
      `💰 ${_afEnLabel} complete! +${fmt(scaledAmt)} raised · equity ${G.equity.toFixed(1)}%${dilLabelEn}`,
      'good'
    );
  } else {
    ...
    pushNewsBI(
      ...
      `🏦 ${_afEnLabel} approved! +${fmt(scaledAmt)} · ${(effectiveRate*100).toFixed(1)}% p.a. (credit ${cr.grade}) · qtrly payment ${fmt(Math.round(qPay))}`,
      'info'
    );
  }
...
function openAcquisitionModal(idx) {
  ...
  const _acqEn0 = typeof LANG !== 'undefined' && LANG.current === 'en';
  const warningHtml = !canAfford
    ? `<div style="background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.3);
         ...
         ⚠ ${_acqEn0 ? `Insufficient cash — need: ${fmt(price)} / have: ${fmt(G.cash)}` : `현금 부족 — 필요: ${fmt(price)} / 보유: ${fmt(G.cash)}`}
       </div>` : '';
  ...
  const cashWarnHtml = canAfford && percentCash > 60
    ? `<div style="background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.3);
         ...
         ⚠ ${_acqEn0 ? `This uses ${percentCash}% of your cash. Check operating funds.` : `현금의 ${percentCash}%를 사용합니다. 운영 자금을 확인하세요.`}
       </div>` : '';
  ...
  const _acqEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('acq-ipo-badge').textContent = c.ipo ? (_acqEn ? `📈 Public (${c.ipoYear})` : `📈 상장사 (${c.ipoYear})`) : '';

  document.getElementById('acq-details').innerHTML = `
    <div class="acq-grid">
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Qtr Revenue':'분기 매출'}</div><div class="acq-s-val">${fmt(c.revenue)}</div></div>
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Net Income':'순이익'}</div><div class="acq-s-val" style="color:${c.profit>=0?'var(--green)':'var(--red)'}">${c.profit>=0?'+':''}${fmt(c.profit)}</div></div>
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Cash':'현금'}</div><div class="acq-s-val">${fmt(c.cash)}</div></div>
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Staff':'직원'}</div><div class="acq-s-val">${c.employees}${_acqEn?'':' 명'}</div></div>
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Quality':'품질'}</div><div class="acq-s-val">${Math.round(c.quality)}/100</div></div>
      <div class="acq-stat"><div class="acq-s-label">${_acqEn?'Tech':'기술력'}</div><div class="acq-s-val">${Math.round(c.tech)}/100</div></div>
    </div>
    <div class="acq-price-section">
      <div class="acq-price-row"><span>${_acqEn?`Valuation (ARR × ${lerp(REV_MULTIPLE,G.year).toFixed(1)}×)`:`기업가치(ARR × ${lerp(REV_MULTIPLE,G.year).toFixed(1)}배)`}</span><span>${fmt(val)}</span></div>
      <div class="acq-price-row"><span>${_acqEn?'Acquisition premium':'인수 프리미엄'}</span><span>× ${premium.toFixed(1)}</span></div>
      <div class="acq-price-row acq-total"><span>${_acqEn?'Total acquisition cost':'총 인수 비용'}</span><span style="color:var(--amber)">${fmt(price)}</span></div>
    </div>
    ${warningHtml}${cashWarnHtml}
    <div class="acq-gain-section">
      <div style="font-size:10px;color:var(--muted);margin-bottom:6px">${_acqEn?'Post-acquisition effects':'인수 후 효과'}</div>
      <div class="acq-gain-row">👥 ${_acqEn?`Staff +${absorbed}`:`직원 +${absorbed}명`} <span style="color:var(--muted);font-size:9px">${_acqEn?'(20% integration churn)':'(인수 이탈 20% 제외)'}</span></div>
      ${qGain > 0 ? `<div class="acq-gain-row">🔵 ${_acqEn?'Quality':'품질'} +${qGain}</div>` : ''}
      ${bGain > 0 ? `<div class="acq-gain-row">🟢 ${_acqEn?'Brand':'브랜드'} +${bGain}</div>` : ''}
      ${tGain > 0 ? `<div class="acq-gain-row">🟣 ${_acqEn?'Tech':'기술력'} +${tGain}</div>` : ''}
      <div class="acq-gain-row" style="color:var(--amber)">💛 ${_acqEn?'Morale -8 (integration friction)':'직원 사기 -8 (통합 진통)'}</div>
    </div>`;
...
function executeAcquisition(idx) {
  ...
  pushNewsBI(
    `🤝 ${c.name} 인수 완료! 비용 ${fmt(price)} · 직원 +${absorbed}명 흡수`,
    `🤝 ${c.name} acquired! Cost ${fmt(price)} · +${absorbed} staff absorbed`,
    'good'
  );
...
function renderFundingStatus() {
  if (!G) return;
  const _fsEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  ...
  if (totalQPay > 0) {
    loanRow.style.display = '';
    document.getElementById('fs-loan-pay').textContent = fmt(totalQPay) + (_fsEn ? '/qtr' : '/분기');
  } else {
    loanRow.style.display = 'none';
  }
...
function launchNewProduct() {
  ...
  if (G.cash < cost) {
    pushNewsBI(`💸 신제품 출시 실패 — 자금 부족 (필요: ${fmt(cost)})`, `💸 New product launch failed — insufficient cash (need ${fmt(cost)})`, 'bad');
    return;
  }
  G.cash -= cost;
  ...
  pushNewsBI(`🚀 ${G.productName} 신버전 출시! 비용 ${fmt(cost)} · 제품 품질 리셋`, `🚀 ${G.productName} new version launched! Cost ${fmt(cost)} · product quality reset`, 'good');
  renderAll();
}
...
function showEventModal(ev) {
  document.getElementById('ev-era-tag').textContent = tEraName(getEraIndex()) + ' ' + t('ev.era.suffix');
  const badge = document.getElementById('ev-type-badge');
  const typeLabels = {
    opportunity: t('ev.type.opportunity'),
    crisis:      t('ev.type.crisis'),
    revolution:  t('ev.type.revolution'),
    boom:        t('ev.type.boom'),
    shift:       t('ev.type.shift'),
  };
  badge.textContent  = typeLabels[ev.type] || ev.type;
  badge.className    = 'modal-type-badge ' + ev.type;
  document.getElementById('ev-title').textContent = tEvent(ev.title,'title') || ev.title;
  document.getElementById('ev-body').textContent  = tEvent(ev.title,'desc')  || ev.desc;
  ...
  if ((ev.type === 'revolution' || ev.type === 'shift') && !ev._catchup && activeDebts.length > 0) {
    let debtTotalFrac = 0;
    const _dwEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    const debtLines = activeDebts.map(d => {
      const periods  = Math.floor(d.quartDebt / 4);
      const addFrac  = +(d.baseFrac * (1 + periods * 0.40)).toFixed(2);
      debtTotalFrac += addFrac;
      return `• ${d.title}: +${Math.round(addFrac * 100)}% (${d.quartDebt}${_dwEn ? ' qtrs accumulated' : '분기 누적'})`;
    }).join('\n');
    const debtWarnEl = document.createElement('div');
    debtWarnEl.id = 'ev-debt-warning';
    debtWarnEl.className = 'tech-debt-event-warning';
    const _isEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    debtWarnEl.innerHTML = _isEn
      ? `⚠️ <strong>${t('ev.debt.warn.title')}</strong><br>${t('ev.debt.warn.body')}<br>` +
        `<pre style="margin:4px 0;font-size:10px;color:var(--amber)">${debtLines}</pre>` +
        `<strong>${t('ev.debt.warn.total', {pct: Math.round(debtTotalFrac * 100)})}</strong>`
  ...
  choicesEl.innerHTML = (ev.choices || []).map((c,i) => {
    ...
    if (totalDisplayCost <= 0) {
      ...
      const pct = c.cost > 0 ? t('ev.cost.pct', {pct: Math.round(c.cost * 100)}) : '';
      const floorNote = isFloorActive ? ` · ${t('ev.cost.floor', {amt: floorM >= 1000 ? (floorM/1000).toFixed(1)+'B' : floorM+'M'})}` : '';
      const debtPct = debtExtra > 0 ? ` ${t('ev.cost.debt', {amt: fmt(debtExtra)})}` : '';
      costLabel = `${t('ev.cost.prefix')} ${fmt(actualCost)}${pct ? ` (${pct}${floorNote})` : ''}${debtPct}` +
        (debtExtra > 0 ? ` ${t('ev.cost.total', {amt: fmt(totalDisplayCost)})}` : '');
    }
    ...
    if (minEmp > 0) {
      ...
      reqParts.push(`<span class="choice-req${staffOk ? '' : ' unmet'}">` +
        `${staffOk ? t('ev.req.emp', {n: minEmp}) + ' ✓' : t('ev.req.emp', {n: minEmp - G.employees}) + ' ✗'}` +
        `</span>`);
    }
    if (!canAfford) {
      reqParts.push(`<span class="choice-req unmet">${t('ev.req.cash')} (${fmt(totalDisplayCost)})</span>`);
    }
    const reqHtml = reqParts.join('');

    // 효과 미리보기
    const effLines = [];
    const _effEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    if (c.effect) {
      const e = c.effect;
      if (e.quality) effLines.push(_effEn ? `Quality ${e.quality > 0 ? '+' : ''}${Math.round(e.quality)}` : `품질 ${e.quality > 0 ? '+' : ''}${Math.round(e.quality)}`);
      if (e.brand)   effLines.push(_effEn ? `Brand ${e.brand > 0 ? '+' : ''}${Math.round(e.brand)}`     : `브랜드 ${e.brand > 0 ? '+' : ''}${Math.round(e.brand)}`);
      if (e.tech)    effLines.push(_effEn ? `Tech ${e.tech > 0 ? '+' : ''}${Math.round(e.tech)}`         : `기술력 ${e.tech > 0 ? '+' : ''}${Math.round(e.tech)}`);
      if (e.employees !== undefined) {
        if (e.employees < 0) {
          ...
          effLines.push(_effEn ? `Staff -${layoffN} (${Math.round(layoffPct*100)}% reduction)` : `직원 -${layoffN}명 (${Math.round(layoffPct*100)}% 감축)`);
        } else if (e.employees > 0) {
          effLines.push(_effEn ? `Staff +${e.employees}` : `직원 +${e.employees}명`);
        }
      }
    }
    ...
    const choiceLabel = tEvent(ev.title, 'choice', i) || c.label;
    return `
  ...
  const _en = typeof LANG !== 'undefined' && LANG.current === 'en';
  if (allBlocked) {
    choicesEl.innerHTML += `
    <button class="choice-btn choice-skip" onclick="applyEventSkip()" style="margin-top:8px;border-color:var(--red);opacity:0.85">
      🚨 ${_en ? 'Cannot respond — passing event due to insufficient cash/staff' : '대응 불가 — 현금·인력 부족으로 이벤트 패스'}
      <span class="choice-cost" style="color:var(--red)">${_en ? 'Extra penalty: brand -3 · quality -2' : '추가 패널티: 브랜드 -3 · 품질 -2'}</span>
      <span class="choice-effects">${_en ? 'Base event effects (market contraction etc.) still apply' : '이벤트 기본 효과(시장 위축 등)는 그대로 적용됨'}</span>
    </button>`;
  }
...
function applyEventSkip() {
  ...
  const evTitleStr = tEvent(ev.title, 'title') || ev.title;
  pushNewsBI(
    `⛔ ${ev.title}: 현금·인력 부족으로 대응 불가 — 브랜드·품질 추가 하락`,
    `⛔ ${evTitleStr}: unable to respond due to insufficient cash/staff — brand & quality decline`,
    'bad'
  );
...
function applyEventChoice(i) {
    ...
    if (e.employees !== undefined) {
      if (e.employees < 0) {
        ...
        pushNewsBI(
          ...
          `✂️ Major restructuring: ${layoff} laid off (${Math.round(layoffPct*100)}%) · severance ${fmt(severance)}`,
          'bad'
        );
      } else if (e.employees > 0) {
  ...
  if (totalCost > 0 && G.cash < totalCost) {
    ...
    pushNewsBI(
      `💸 현금 부족 — 기존 기술 부채 포함 총 ${fmt(totalCost)} 필요`,
      `💸 Insufficient cash — total ${fmt(totalCost)} needed (including existing tech debt)`,
      'bad'
    );
    return;
  }
  ...
  if (debtClearCost > 0) {
    ...
    pushNewsBI(
      `💸 기술 부채 자동 청산 (+${fmt(debtClearCost)}) — ${resolved.length}건 해소`,
      `💸 Tech debt auto-settled (+${fmt(debtClearCost)}) — ${resolved.length} debt(s) cleared`,
      'bad'
    );
  }
  ...
  if (ev._catchup) {
    ...
    if (debt) {
      if (ch._resolveFull) {
        debt.resolved = true;
        pushNewsBI(`✅ ${debt.title} 기술 부채 완전 해소!`, `✅ ${debt.title} tech debt fully resolved!`, 'good');
        ...
        pushNewsBI(`⚡ ${debt.title} 부분 추격 완료 — 기술 부채 절반 이상 감소`, `⚡ ${debt.title} partially caught up — tech debt reduced by over half`, 'good');
      } else {
        pushNewsBI(`⏳ ${debt.title} 추격 연기 — 4분기 후 더 비싸진 기회가 제공됩니다`, `⏳ ${debt.title} catch-up deferred — a costlier opportunity arrives in 4 quarters`, 'warn');
      }
    }
  }
  ...
  if ((ev.type === 'revolution' || ev.type === 'shift') && !ev._catchup) {
    ...
    if (missedFrac > 0.04) {
      ...
      const severityEn = missedFrac > 0.6 ? 'Critical' : missedFrac > 0.2 ? 'Significant' : 'Minor';
      const evTitleTD  = tEvent(ev.title, 'title') || ev.title;
      pushNewsBI(
        `🕰️ ${ev.title}: ${severity} 기술 부채 발생 (미투자 ${Math.round(missedFrac*100)}%) — 매 분기 품질·기술·브랜드 지속 하락`,
        `🕰️ ${evTitleTD}: ${severityEn} tech debt incurred (under-invested ${Math.round(missedFrac*100)}%) — quality, tech & brand declining each quarter`,
  ...
  const costStrEn = totalCost > 0 ? ` · cost ${fmt(totalCost)}` : '';
  const evTitleFn  = tEvent(ev.title, 'title') || ev.title;
  const chLabelEn  = tEvent(ev.title, 'choice', i) || ch.label;
  pushNewsBI(
    `⚡ ${ev.title}: "${ch.label.replace(/[^\w가-힣 ·!]/g,'').trim()}" 선택${costStr}`,
    `⚡ ${evTitleFn}: chose "${chLabelEn.replace(/[^\w\s·!,-]/g,'').trim()}"${costStrEn}`,
    'info'
  );
...
function showResultsModal(info) {
  ...
  document.getElementById('res-q').textContent   = `Q${prevQ} ${prevY} ${t('res.title.suffix')}`;
  document.getElementById('res-era').textContent  = tEraName(getEraIndex());
  ...
  if (eraFin && h.rev > 0) {
    ...
    if (margin >= ee)      marginTag = t('res.margin.elite') + repStr;
    else if (margin >= eg) marginTag = t('res.margin.good', {pct: eg});
    else if (margin >= 0)  marginTag = t('res.margin.ok', {pct: eg});
    else if (margin >= ew) marginTag = t('res.margin.loss');
    else                   marginTag = t('res.margin.severe');
  }

  const empUnit = LANG.current === 'en' ? '' : '명';
  const grid = [
    { l: t('res.rev'),     v:fmt(h.rev),    d: prev ? delta(h.rev, prev.rev) : null },
    { l: t('res.profit'),  v:`${fmt(h.profit)} <span style="font-size:11px;opacity:.7">(${margin}%)</span>`, d: null },
    { l: t('res.cash'),    v:fmt(h.cash),   d: null },
    { l: t('res.share'),   v:_myRel.toFixed(1)+'%', d: prev ? shareDelta(h.share,prev.share) : null },
    { l: t('res.emp'),     v:h.emp+(empUnit ? empUnit : ''), d: null },
    { l: t('res.quality'), v:Math.round(h.quality)+'/100', d: null },
  ];
  ...
  const noNewsStr = LANG.current === 'en' ? 'Nothing notable.' : '특이사항 없음';
  const newsHtml = news.map(n=>`<div>• ${n.t}: ${n.msg}</div>`).join('') || noNewsStr;
...
function showBriefingModal() {
  ...
  const _sbEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('brief-mood').textContent  = moodEmoji;
  const _briefQtr = G.history.length > 0 ? (((G.history.length-1) % 4) + 1) : G.quarter;
  document.getElementById('brief-date').textContent  =
    `Q${_briefQtr} ${G.year} — ${_sbEn ? 'Quarterly Briefing' : '분기 브리핑'}`;

  // 탭 콘텐츠 생성
  _briefTabs = [
    { label: _sbEn ? '📊 Results'    : '📊 실적', html: _briefPerformance(h, prev, mood) },
    { label: _sbEn ? '⚔️ Competition' : '⚔️ 경쟁', html: _briefCompetitors(h) },
    { label: _sbEn ? '💰 Finance'    : '💰 자금', html: _briefFinance(h) },
    { label: _sbEn ? '🧭 Strategy'   : '🧭 전략', html: _briefStrategy(h, prev, mood) },
  ];
...
function _briefPerformance(h, prev, mood) {
  ...
  const _bpEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const kpiCards = [
    { label: _bpEn ? 'Qtr Revenue'      : '분기 매출',
      val: fmt(h.rev),
      delta: revDelta !== null ? (revDelta >= 0 ? `▲ ${revDelta}%` : `▼ ${Math.abs(revDelta)}%`) : null,
      pos: revDelta >= 0 },
    { label: _bpEn ? 'Net Income (Margin)' : '순이익 (마진)',
      val: `${fmt(h.profit)} (${margin}%)`,
      delta: margin >= 0 ? (_bpEn ? 'Profit' : '흑자') : (_bpEn ? 'Loss' : '적자'), pos: margin >= 0 },
    { label: _bpEn ? 'Market Share'     : '시장점유율',
      val: myRel.toFixed(1) + '%',
      delta: shareDelta !== null ? (shareDelta >= 0 ? `▲ ${shareDelta}%p` : `▼ ${Math.abs(shareDelta)}%p`) : null,
      pos: shareDelta >= 0 },
    { label: _bpEn ? 'Cash Balance'     : '현금 잔고',
      val: fmt(h.cash), delta: null, pos: true },
  ];
  ...
  const commentLines = [];
  if (mood === 'excellent')
    commentLines.push(_bpEn
      ? `Outstanding quarter, CEO. Net margin of <strong>${margin}%</strong> is top-tier for the industry.`
      : `이번 분기도 매우 훌륭한 실적입니다, 대표님. 영업이익률 <strong>${margin}%</strong>는 업계 최상위 수준이에요.`);
  else if (mood === 'good')
    commentLines.push(_bpEn
      ? `Solid quarter. Revenue <strong>${fmt(h.rev)}</strong>, up ${revDelta !== null ? revDelta + '%' : '—'} vs. prior quarter.`
      : `안정적인 분기였습니다. 매출 <strong>${fmt(h.rev)}</strong>로 전분기 대비 ${revDelta !== null ? revDelta + '%' : '—'} 성장했어요.`);
  else if (mood === 'neutral')
    commentLines.push(_bpEn
      ? `Profitability is on the low side. Review cost structure or consider adjusting price positioning.`
      : `수익성이 다소 낮습니다. 비용 구조를 점검하거나 가격 포지셔닝 조정을 검토해 보세요.`);
  else if (mood === 'concern')
    commentLines.push(_bpEn
      ? `Losses are continuing. Watch your cash burn rate. Check the Funding tab.`
      : `적자가 지속되고 있습니다. 현금 소진 속도에 주의가 필요합니다. 자금 탭을 확인해 주세요.`);
  else
    commentLines.push(_bpEn
      ? `⚠️ CEO, the situation is critical. Immediate cost cuts and financing are required.`
      : `⚠️ 대표님, 상황이 위급합니다. 즉각적인 비용 절감과 자금 조달이 필요합니다.`);

  if (prev && (h.relShare ?? h.share) < (prev.relShare ?? prev.share))
    commentLines.push(_bpEn
      ? `Market share dropped <strong>${(Math.abs((h.relShare??h.share) - (prev.relShare??prev.share))*100).toFixed(1)}%p</strong>. Check the Competitors tab.`
      : `시장점유율이 <strong>${(Math.abs((h.relShare??h.share) - (prev.relShare??prev.share))*100).toFixed(1)}%p</strong> 하락했습니다. 경쟁사 탭을 확인해 주세요.`);
  if (h.quality >= 80)
    commentLines.push(_bpEn
      ? `Product quality score <strong>${Math.round(h.quality)}/100</strong> — customer satisfaction remains high.`
      : `제품 품질 점수 <strong>${Math.round(h.quality)}/100</strong> — 고객 만족도가 높게 유지되고 있습니다.`);

  return `
    <div class="brief-section">
      <div class="brief-section-title">${_bpEn ? 'Key Metrics' : '핵심 지표'}</div>
      ${kpiHtml}
    </div>
...
function _briefCompetitors(h) {
  ...
  const _bcEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  ...
  const tableHtml = alive.length > 0 ? `
    <table class="brief-comp-table">
      <thead><tr>
        <th>${_bcEn ? 'Competitor' : '경쟁사'}</th><th>${_bcEn ? 'Peer Share' : '상대 점유율'}</th><th>${_bcEn ? 'Strategy' : '전략'}</th><th>${_bcEn ? 'Threat' : '위협'}</th>
      </tr></thead>
      <tbody>${sortedAlive.map(c => {
        ...
        return `<tr class="${rowClass}">
          ...
          <td style="font-size:10px">${(c.quality || 0) > (h.quality || 0) ? (_bcEn ? '🔴 Quality edge' : '🔴 품질우위') : '🟢'}</td>
        </tr>`;
      }).join('')}</tbody>
    </table>` : `<div class="brief-ok">✅ ${_bcEn ? 'No active competitors. You own the market.' : '현재 살아있는 경쟁사가 없습니다. 시장을 독점하고 있어요.'}</div>`;
  ...
  const acquireHtml = acquirables.length > 0 ? `
    <div class="brief-section">
      <div class="brief-section-title">💡 ${_bcEn ? 'Acquisition Targets' : '인수 검토 대상'}</div>
      ${acquirables.slice(0, 2).map(c => {
        ...
        return `<div class="brief-acquire-card">
          ...
          <div class="brief-acquire-desc">${_bcEn
            ? `Peer share ${toRel(c.marketShare||0).toFixed(1)}% — weaker than half your share. Acquisition yields tech & customer absorption.`
            : `상대 점유율 ${toRel(c.marketShare||0).toFixed(1)}% — 우리 대비 절반 이하의 약체 경쟁사입니다. 인수 시 기술·고객 흡수 가능.`}</div>
          <div class="brief-acquire-price">💰 ${_bcEn ? `Est. acquisition cost ${fmt(estPrice)} (selectable when event fires)` : `추정 인수가 ${fmt(estPrice)} (이벤트 발생 시 선택 가능)`}</div>
        </div>`;
      }).join('')}
    </div>` : '';
  ...
  const comment = topThreat
    ? (_bcEn
        ? `The strongest competitor right now is <strong>${topThreat.name}</strong> (peer share ${toRel(topThreat.marketShare||0).toFixed(1)}%).${
            (topThreat.quality || 0) > (h.quality || 0) ? ' They also have a quality edge — consider boosting R&D investment.' : ''}`
    ...
    : (_bcEn ? 'Your market position is stable.' : '현재 시장에서 우리 회사의 위치가 안정적입니다.');

  return `
    <div class="brief-section">
      <div class="brief-section-title">${_bcEn ? `Competitor Overview — my peer share ${myRel.toFixed(1)}%` : `경쟁사 현황 — 내 상대 점유율 ${myRel.toFixed(1)}%`}</div>
      ${tableHtml}
    </div>
    ${acquireHtml}
    <div class="brief-bubble">${comment}${dead.length > 0 ? `<br>${_bcEn ? `${dead.length} competitor(s) already eliminated from the market.` : `이미 시장에서 퇴출된 경쟁사 ${dead.length}개사.`}` : ''}</div>`;
}
  ...
  const loanHtml = loans.length > 0 ? loans.map(l => {
    ...
    return `<div class="brief-fund-row">
      <div class="brief-fund-label">
        <span>${loanName}</span>
        <span style="color:var(--amber)">${fmt(remaining)} (${l.quartersLeft || 0}${_bfEn ? ' qtrs left' : '분기 잔여'} · ${_bfEn ? 'qtrly' : '분기'} ${fmt(l.qPay || 0)})</span>
      </div>
    </div>`;
  }).join('') : `<div class="brief-ok">✅ ${_bfEn ? 'No outstanding debt' : '현재 부채 없음'}</div>`;

  // 비서 코멘트
  let comment = '';
  if (h.cash < 50000)
    comment = _bfEn
      ? `⚠️ <strong>Cash is at danger level</strong> (${fmt(h.cash)}). Seek funding immediately.`
      : `⚠️ <strong>현금이 위험 수위입니다</strong> (${fmt(h.cash)}). 즉시 자금 조달을 검토하세요.`;
  else if (cashStatus === 'warn')
    comment = _bfEn
      ? `Cash reserves are somewhat low. Consider funding within 2–3 quarters.`
      : `현금 보유량이 다소 낮습니다. 향후 2~3분기 내 자금 조달을 고려해 주세요.`;
  else if (debtRatio > 0.5)
    comment = _bfEn
      ? `High debt ratio (${Math.round(debtRatio*100)}% of annual revenue). Review repayment schedule.`
      : `부채 비율이 높습니다 (연매출 대비 ${Math.round(debtRatio*100)}%). 상환 일정을 점검하세요.`;
  else
    comment = _bfEn
      ? `Financials are stable. Cash <strong>${fmt(h.cash)}</strong> on hand; quarterly repayment ${qPay > 0 ? fmt(qPay) : 'none'}.`
      : `자금 상황은 안정적입니다. 현금 <strong>${fmt(h.cash)}</strong> 보유 중이며 분기 상환액은 ${qPay > 0 ? fmt(qPay) : '없음'}입니다.`;

  return `
    <div class="brief-section">
      <div class="brief-section-title">${_bfEn ? 'Cash Status' : '현금 상태'}</div>
      <div class="brief-fund-row">
        <div class="brief-fund-label">
          <span>${_bfEn ? 'Cash balance' : '현금 잔고'}</span>
    ...
    <div class="brief-section">
      <div class="brief-section-title">${_bfEn ? `Loans (total balance ${fmt(totalDebt)} · qtrly payment ${qPay > 0 ? fmt(qPay) : 'none'})` : `대출 현황 (총 잔액 ${fmt(totalDebt)} · 분기 상환 ${qPay > 0 ? fmt(qPay) : '없음'})`}</div>
      ${loanHtml}
    </div>
...
function _briefStrategy(h, prev, mood) {
  ...
  // 경영이론 선택 + 전략 추천
  const _bsEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const strategies = [];
  const eraNameBS  = tEraName(getEraIndex());

  // 1. Porter's Generic Strategies
  if (myRel > 0.4) {
    strategies.push({
      theory: _bsEn ? "Porter's Competitive Advantage" : "Porter의 경쟁우위론",
      title:  _bsEn ? "Defend Market Leadership"        : "시장 리더십 방어",
      body:   _bsEn
        ? `Leading at ${(myRel*100).toFixed(0)}% peer share. Sustain <strong>cost leadership</strong> or <strong>differentiation</strong> to raise entry barriers.`
        : `현재 ${(myRel*100).toFixed(0)}% 상대 점유율로 시장을 선도하고 있습니다. <strong>비용 리더십</strong> 또는 <strong>차별화</strong> 전략을 유지하여 진입장벽을 높이세요.`,
    });
  } else if (myRel < 0.15) {
    strategies.push({
      theory: _bsEn ? "Porter's Focus Strategy"           : "Porter의 집중화 전략",
      title:  _bsEn ? "Niche Focus — Own a Small Market"  : "틈새 집중 — 작은 시장 지배",
      body:   _bsEn
        ? `Low share means a <strong>Focus strategy</strong> is most effective now. Serve a specific customer segment with the best product and expand gradually.`
        : `점유율이 낮은 지금은 <strong>집중화(Focus) 전략</strong>이 유효합니다. 특정 고객군에 집중해 최고의 제품을 제공하고, 점유율을 점진적으로 확장하세요.`,
    });
  }
  ...
  if (era && era.year >= 2007) {
    strategies.push({
      theory: _bsEn ? "Disruptive Innovation (Christensen)"       : "파괴적 혁신 이론 (Christensen)",
      title:  _bsEn ? `${eraNameBS} — Disrupt or be Disrupted`    : `${eraNameBS} — 파괴자가 되거나 파괴당하거나`,
      body:   _bsEn
        ? `Technology transitions are rapid. Secure tech advantage via <strong>R&D investment</strong>, or quickly absorb capabilities through <strong>competitor M&A</strong>.`
        : `현재 시대는 기술 전환이 빠릅니다. <strong>R&D 투자 강화</strong>로 기술 우위를 확보하거나, <strong>기존 경쟁사 M&A</strong>로 빠르게 역량을 흡수하는 전략을 고려하세요.`,
    });
  }
  ...
  if (margin < -0.05 && G.cash > 300000) {
    strategies.push({
      theory: _bsEn ? "Blitzscaling (Hoffman)"           : "블리츠스케일링 (Hoffman)",
      title:  _bsEn ? "Growth First — Losses Are Investment" : "성장 우선 — 적자는 투자다",
      body:   _bsEn
        ? `Operating at a loss (margin ${Math.round(margin*100)}%) but holding ${fmt(G.cash)} cash. In an early growth phase, <strong>aggressive marketing investment</strong> to capture share is rational.`
        : `현재 적자(마진 ${Math.round(margin*100)}%)이지만 현금 ${fmt(G.cash)} 보유 중. 초기 성장 단계라면 점유율 확보를 위한 <strong>공격적 마케팅 투자</strong>가 합리적입니다.`,
    });
  } else if (margin < 0 && G.cash < 200000) {
    strategies.push({
      theory: _bsEn ? "Cash Flow First Principle"     : "현금흐름 우선 원칙",
      title:  _bsEn ? "⚠️ Switch to Survival Mode"   : "⚠️ 생존 모드 전환 필요",
      body:   _bsEn
        ? `Cash is critically low at ${fmt(G.cash)}. Prioritize <strong>cost cuts (HR/marketing)</strong> and a <strong>price increase</strong> to turn cash flow positive immediately.`
        : `현금이 ${fmt(G.cash)}로 부족합니다. 당장 <strong>비용 절감(HR/마케팅 축소)</strong>과 <strong>가격 인상</strong>으로 현금흐름을 플러스로 전환하는 것이 최우선입니다.`,
    });
  }
  ...
  if (prev && h.share > prev.share * 1.05) {
    strategies.push({
      theory: _bsEn ? "Network Effects & Economies of Scale" : "네트워크 효과 & 규모의 경제",
      title:  _bsEn ? "Growth Momentum — Time to Accelerate" : "성장 모멘텀 — 가속 투자 시점",
      body:   _bsEn
        ? `Market share is rising fast. Increasing <strong>marketing spend</strong> now expands your customer base, lowers unit costs, and creates a virtuous cycle.`
        : `점유율이 빠르게 상승 중입니다. 이 모멘텀을 활용해 <strong>마케팅 예산을 증가</strong>시키면 고객 기반이 확대되어 단위 비용이 낮아지는 선순환이 만들어집니다.`,
    });
  }
  ...
  if (h.quality < 50) {
    strategies.push({
      theory: _bsEn ? "Quality Management (Deming / TQM)" : "품질 경영 (Deming / TQM)",
      title:  _bsEn ? "Product Quality Crisis"            : "제품 품질 위기",
      body:   _bsEn
        ? `Product quality is <strong>${Math.round(h.quality)}/100</strong> — leading to churn and NPS decline. Boost R&D and HR budgets to make <strong>quality recovery the top priority</strong>.`
        : `현재 제품 품질 <strong>${Math.round(h.quality)}/100</strong>. 이는 고객 이탈과 NPS 하락으로 이어집니다. R&D 및 HR 예산을 늘려 <strong>품질 회복을 최우선</strong>으로 삼으세요.`,
    });
  }
  ...
  if (strategies.length === 0) {
    strategies.push({
      theory: _bsEn ? "Ansoff Growth Matrix"                 : "앤소프 성장 매트릭스",
      title:  _bsEn ? "Stable Growth — Market Penetration"   : "안정적 성장 — 시장 침투 전략",
      body:   _bsEn
        ? `All indicators are stable. Focus on <strong>increasing penetration of your existing product</strong>. Boost marketing to grow share and consolidate your position.`
        : `현재 지표가 안정적입니다. <strong>기존 제품의 시장 침투율 향상</strong>에 집중하세요. 마케팅 강화로 점유율을 높이고, 내실을 다지는 시기입니다.`,
    });
  }
  ...
  const stratNames  = _bsEn
    ? { differentiation:'Differentiation', cost_leadership:'Cost Leadership', focus:'Focus' }
    : { differentiation:'차별화', cost_leadership:'비용 리더십', focus:'집중화' };

  return `
    <div class="brief-section">
      <div class="brief-section-title">${_bsEn ? 'Current Era' : '현재 시대'} — ${eraNameBS}</div>
      <div class="brief-kpi-row">
        <div class="brief-kpi" style="min-width:unset">
          <div class="brief-kpi-label">${_bsEn ? 'My Strategy' : '내 전략'}</div>
          <div class="brief-kpi-val" style="font-size:13px">${stratNames[curStrategy] || curStrategy}</div>
        </div>
        <div class="brief-kpi" style="min-width:unset">
          <div class="brief-kpi-label">${_bsEn ? 'Net Margin' : '이익률'}</div>
          <div class="brief-kpi-val" style="font-size:13px;color:${margin>=0?'var(--green)':'var(--red)'}">${Math.round(margin*100)}%</div>
        </div>
        <div class="brief-kpi" style="min-width:unset">
          <div class="brief-kpi-label">${_bsEn ? 'Peer Share' : '상대 점유율'}</div>
          <div class="brief-kpi-val" style="font-size:13px">${(myRel*100).toFixed(1)}%</div>
        </div>
      </div>
    </div>
    <div class="brief-section">
      <div class="brief-section-title">${_bsEn ? 'Strategic Recommendations' : '전략 제언'}</div>
      ${stratHtml}
    </div>`;
}
...
function showGameOver() {
  ...
  const win  = G.win;
  const _en  = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('go-label').textContent = win
    ? (_en ? '🎉 Complete!' : '게임 클리어!')
    : (_en ? 'Game Over'   : '게임 오버');
  const titleEl = document.getElementById('go-title');
  titleEl.textContent = win ? (_en ? 'Empire Built!' : '제국 완성!') : (_en ? 'Bankrupt' : '파산');
  titleEl.className   = 'go-title ' + (win ? 'win' : 'lose');
  document.getElementById('go-sub').textContent = win
    ? (_en
        ? `${G.name} survived to 2050 and built a global software empire!`
        : `${G.name}이 2050년까지 생존하여 소프트웨어 제국을 완성했습니다!`)
    : (_en
        ? `${G.name} ran out of cash and went bankrupt. (${G.year} Q${G.quarter})`
        : `${G.name}의 현금이 소진되어 파산했습니다. (${G.year}년 Q${G.quarter})`);

  const grid = _en
    ? [
        { l:'Peak Qtr Revenue',   v: fmt(G.peakRevenue) },
        { l:'Peak Annual Run Rate', v: fmt(G.peakRevenue*4) },
        { l:'Peak Market Share',  v: (G.peakMarketShare * 100).toFixed(1) + '%' },
        { l:'Peak Cash Balance',  v: fmt(G.peakCash) },
      ]
    : [
  ...
  const evals  = _en
    ? [
        ['Annual revenue $1B+ achieved',   G.peakRevenue*4 >= 1e9],
        ['Annual revenue $100B+ achieved', G.peakRevenue*4 >= 1e11],
        ['Peer market share ≥ 10%',        G.peakMarketShare >= 0.10],
        ['Peer market share ≥ 50%',        G.peakMarketShare >= 0.50],
        ['All competitors bankrupted',     G.competitors.every(c=>!c.alive)],
        [`Survived ${Math.round(yrsRun)}+ years`, yrsRun >= 20],
      ]
    : [
  ...
  document.getElementById('go-eval').innerHTML =
    `<strong style="display:block;margin-bottom:8px">${_en ? 'Achievements' : '달성 목표'}</strong>` +
...
function _updateHireBar() {
  ...
  const _uhbEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  if (curEl) curEl.textContent = cur.toLocaleString() + (_uhbEn ? '' : '명');
  if (minEl) minEl.textContent = minEmp.toLocaleString() + (_uhbEn ? '' : '명');
  if (maxEl) maxEl.textContent = '+' + maxHire.toLocaleString() + (_uhbEn ? '' : '명');
}
...
function promptPrice() {
  const _ppEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const cur = Math.round((decisionState.price || 1.0) * 100);
  const val = window.prompt(
    _ppEn ? `Enter price positioning (60–160%)\nCurrent: ${cur}%` : `가격 포지셔닝 입력 (60~160%)\n현재: ${cur}%`,
    cur);
  if (val === null) return;
...
function promptBudget(key) {
  const _pbEn  = typeof LANG !== 'undefined' && LANG.current === 'en';
  const labels = _pbEn
    ? { rd: 'R&D', mkt: 'Marketing', hr: 'HR' }
    : { rd: 'R&D', mkt: '마케팅', hr: 'HR' };
  ...
  const _promptMsg = _pbEn
    ? `Enter ${labels[key]} budget (in $K)\nCurrent: ${fmt(cur)}\nMax: ${fmt(max)}`
    : `${labels[key]} 예산 입력 ($K 단위)\n현재: ${fmt(cur)}\n최대: ${fmt(max)}`;
  const val = window.prompt(_promptMsg, Math.round(cur / 1000));
  if (val === null) return;
  const n = parseInt(val);
...
function syncSlider(key) {
  ...
  if (key === 'hr') {
    ...
    if (hintEl) {
      const _hrEn  = typeof LANG !== 'undefined' && LANG.current === 'en';
      ...
      const idxLabel = _hrEn
        ? (dispIdx < 0.5 ? 'None' : dispIdx < 1.0 ? 'Low' : dispIdx < 2.0 ? 'Adequate' : dispIdx < 3.5 ? 'Aggressive' : 'Max')
        : (dispIdx < 0.5 ? '미투자' : dispIdx < 1.0 ? '부족' : dispIdx < 2.0 ? '적정' : dispIdx < 3.5 ? '공격적' : '최대');
      ...
      hintEl.innerHTML = _hrEn
        ? `HR Index <b>${dispIdx.toFixed(1)}×</b> (${idxLabel}) &nbsp;|&nbsp; ` +
          `Attrition <b style="color:${ef.attrAdj<=0?'#10b981':'#ef4444'}">${attrPpt>=0?'+':''}${attrPpt}%p</b> &nbsp;` +
          `Recruitment <b style="color:${recPct>=0?'#10b981':'#ef4444'}">${recPct>=0?'+':''}${recPct}%</b> &nbsp;|&nbsp; ` +
          `Eff. Wage <b style="color:${wageColor}">×${cWage.toFixed(2)} mkt</b> <span style="color:var(--muted);font-size:10px">(total ${totalWage>=0?'+':''}${totalWage}%)</span>`
...
function syncPrice() {
  ...
  const _prEn    = typeof LANG !== 'undefined' && LANG.current === 'en';
  const shareChg = Math.round((shareFactor - 1) * 100);
  const revChg   = Math.round((revFactor   - 1) * 100);
  const shareStr = _prEn
    ? (shareChg >= 0 ? `share +${shareChg}%` : `share ${shareChg}%`)
    : (shareChg >= 0 ? `점유율 +${shareChg}%` : `점유율 ${shareChg}%`);
  const revStr   = _prEn
    ? (revChg >= 0 ? `revenue +${revChg}%` : `revenue ${revChg}%`)
    : (revChg >= 0 ? `매출 +${revChg}%` : `매출 ${revChg}%`);

  let hint;
  if (_prEn) {
    if (v <= 0.75)      hint = `Ultra Low — ${shareStr} / ${revStr} (razor-thin margin)`;
    else if (v <= 0.90) hint = `Discount — ${shareStr} / ${revStr}`;
    else if (v <= 1.10) hint = `Market Rate — balanced share & revenue`;
    else if (v <= 1.30) hint = `Premium — ${shareStr} / ${revStr} (high margin)`;
    else                hint = `Ultra High — ${shareStr} / ${revStr} ⚠ elasticity risk`;
  } else {
...
function _refreshFillButton() {
  ...
  const _fbEn   = typeof LANG !== 'undefined' && LANG.current === 'en';
  const minEmp  = calcMinEmployees();
  const gap     = Math.max(0, minEmp - G.employees);
  const pending = decisionState.hire || 0;
  if (gap > 0 && pending < gap) {
    ...
    document.getElementById('fill-min-label').textContent = `+${fill.toLocaleString()}${_fbEn ? '' : '명'}`;
    document.getElementById('fill-min-sub').textContent   = capped
      ? (_fbEn ? ` (qtr limit — ${gap.toLocaleString()} total needed)` : ` (분기 한도 — 총 ${gap.toLocaleString()}명 필요)`)
      : (_fbEn ? ` (target: ${minEmp.toLocaleString()})` : ` (${minEmp.toLocaleString()}명 목표)`);
...
function _refreshHireDisplay() {
  ...
  const _hcEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const hcUnit = document.getElementById('hire-count-unit');
  if (hcUnit) hcUnit.textContent = _hcEn ? '' : '명';
  const compWage = getCompanyWageMult(G, true);
  const hrIdx    = getHrIndex(decisionState.hr, G.employees, G.year);
  const wagePrem = getHrEffects(hrIdx).wagePremium;
  const hireCost = Math.max(0, decisionState.hire) * getSalary(G.year) / 4 * compWage * (1 + wagePrem) * 0.3;
  const _rdEn   = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('hire-cost-txt').textContent = (_rdEn ? 'Hiring cost ' : '채용비 ') + fmt(hireCost);
...
function renderHireButtons() {
  ...
  const _hbEn2 = typeof LANG !== 'undefined' && LANG.current === 'en';
  _updateHireBar();
  const maxHire = getMaxHire();
  const maxFire = Math.max(3, Math.floor(G.employees * 0.25));
  ...
  // Update hire stats hint line with bilingual text
  const hireStatsEl = document.getElementById('hire-stats-hint');
  if (hireStatsEl) {
    const minEmpH = calcMinEmployees();
    hireStatsEl.innerHTML = _hbEn2
      ? `Current: <span id="cur-emp">${G.employees}</span> · Max hire <span id="hire-max-disp">${maxHire}</span> · Min required: <span id="min-emp-disp" style="color:${G.employees<minEmpH?'var(--red)':'var(--green)'}">${minEmpH.toLocaleString()}</span> · <span id="emp-cap-disp" style="color:var(--muted)">10,000 cap</span>`
      : `현재: <span id="cur-emp">${G.employees}</span>명 · 최대 <span id="hire-max-disp">${maxHire}</span>명 채용 · 필요 최소: <span id="min-emp-disp" style="color:${G.employees<minEmpH?'var(--red)':'var(--green)'}">${minEmpH.toLocaleString()}</span>명 · <span id="emp-cap-disp" style="color:var(--muted)">10,000명 한도</span>`;
  }
  ...
  if (capEl) {
    const _hbEn    = typeof LANG !== 'undefined' && LANG.current === 'en';
    const minNeeded = calcMinEmployees();
    const shortage  = minNeeded - G.employees;
    if (shortage > 0) {
      capEl.textContent = _hbEn
        ? `Need ${shortage.toLocaleString()} more staff (minimum)`
        : `최소 ${shortage.toLocaleString()}명 추가 채용 필요`;
      capEl.style.color = shortage > G.employees * 0.3 ? 'var(--red)' : 'var(--amber)';
    } else {
      capEl.textContent = _hbEn ? 'Staffing OK' : '인원 충족';
...
function showCatchupModal(debtIdx) {
  ...
  const _cuEn     = typeof LANG !== 'undefined' && LANG.current === 'en';
  const multLabel = (fullFrac / debt.baseFrac).toFixed(2);
  const qWarning  = debt.quartDebt >= 8
    ? (_cuEn
        ? ` ⚠️ ${debt.quartDebt}-quarter delay — costs will spike further at the next revolution event!`
  ...
  pendingEvent = {
    ...
    type: 'shift',
    title: _cuEn
      ? `⏪ ${debt.title} — Catch-Up Investment Opportunity`
      : `⏪ ${debt.title} — 추격 투자 기회`,
    desc: _cuEn
      ? `You delayed investing in ${debt.title}, falling ${debt.quartDebt} quarters (~${(debt.quartDebt/4).toFixed(1)} yrs) behind. ` +
        `Catching up now costs ${multLabel}× the original investment.` +
        `${qWarning}\n\nWaiting until the next opportunity (4 qtrs) will cost even more.`
      : `${debt.title}에 투자를 미뤄 ${debt.quartDebt}분기(약 ${(debt.quartDebt/4).toFixed(1)}년)간 ` +
        `기술이 뒤처졌습니다. 초기 투자 대비 ${multLabel}배 비용이 필요합니다.` +
        `${qWarning}\n\n지금 투자하지 않으면 다음 기회(4분기 후)는 더 비쌉니다.`,
    effects: {},
    choices: [
      {
        label: _cuEn ? `🚀 Full catch-up — resolve tech debt completely` : `🚀 전면 추격 투자 — 기술 부채 완전 해소`,
        cost:   fullFrac,
        effect: { quality: qGain, tech: tGain, brand: bGain },
        _resolveFull: true,
      },
      {
        label: _cuEn ? `⚡ Partial catch-up (50% investment) — reduce tech debt 50%` : `⚡ 부분 추격 (절반 투자) — 기술 부채 50% 감소`,
        cost:   partialFrac,
        effect: { quality: Math.round(qGain * 0.50), tech: Math.round(tGain * 0.50), brand: Math.round(bGain * 0.50) },
        _resolvePartial: true,
      },
      {
        label: _cuEn ? `⏳ Defer to next opportunity — costs keep rising` : `⏳ 다음 기회까지 연기 — 비용 계속 증가`,
        cost:   0,
        effect: {},
      },
    ],
...
function preQuarterCheck() {
  ...
  const _pqEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const budgetLines = [
    `R&D: <strong>${fmt(ds.rd)}</strong>${rdDelta !== null  ? ` <span style="color:${rdDelta  >= 0?'var(--green)':'var(--red)'}">(${rdDelta  >= 0?'+':''}${rdDelta}%)</span>`  : ''}`,
    `${_pqEn ? 'Marketing' : '마케팅'}: <strong>${fmt(ds.mkt)}</strong>${mktDelta !== null ? ` <span style="color:${mktDelta >= 0?'var(--green)':'var(--red)'}">(${mktDelta >= 0?'+':''}${mktDelta}%)</span>` : ''}`,
    `HR: <strong>${fmt(ds.hr)}</strong>${hrDelta !== null  ? ` <span style="color:${hrDelta  >= 0?'var(--green)':'var(--red)'}">(${hrDelta  >= 0?'+':''}${hrDelta}%)</span>`  : ''}`,
  ];
  items.push({ icon: '💰', title: t('pq.budget'), body: budgetLines.join(' &nbsp;·&nbsp; '), warn: false });
  ...
  const empSuffix = LANG.current === 'en' ? '' : '명';
  const hrTitle   = LANG.current === 'en' ? 'Staffing' : '인원 현황';
  const hrWarnTitle = LANG.current === 'en' ? '⚠️ Understaffed' : '인원 부족 경고';
  if (gap > 0 && afterHire < minEmp) {
    items.push({
      icon: '⚠️', title: hrWarnTitle, warn: true,
      body: LANG.current === 'en'
        ? `Need <strong>${minEmp}</strong> / Have <strong>${G.employees}</strong> + hiring <strong>${hiring}</strong> = ${afterHire} (still <strong>${minEmp - afterHire} short</strong>). Revenue penalty will apply.`
        : `최소 필요 <strong>${minEmp}명</strong> / 현재 <strong>${G.employees}명</strong> + 채용 예정 <strong>${hiring}명</strong> = ${afterHire}명 (여전히 <strong>${minEmp - afterHire}명 부족</strong>).<br>인력 부족 시 매출 페널티가 발생합니다.`,
    });
  } else if (gap > 0 && afterHire >= minEmp) {
    items.push({
      icon: '👥', title: hrTitle, warn: false,
      body: LANG.current === 'en'
        ? `Have <strong>${G.employees}</strong> + hiring <strong>${hiring}</strong> → ${afterHire} (min ${minEmp}) — ${t('pq.hr.after')}`
        : `현재 <strong>${G.employees}명</strong> + 채용 예정 <strong>${hiring}명</strong> → ${afterHire}명 (필요 최소 ${minEmp}명) — ${t('pq.hr.after')}`,
    });
  } else {
    items.push({
      icon: '👥', title: hrTitle, warn: false,
      body: LANG.current === 'en'
        ? `Have <strong>${G.employees}</strong> (min ${minEmp}) — ${t('pq.hr.ok')}`
        : `현재 <strong>${G.employees}명</strong> (필요 최소 ${minEmp}명) — ${t('pq.hr.ok')}`,
    });
  }
  ...
  const avgCompPrice = alive.length
    ...
    : 1.0;
  const priceLabel = LANG.current === 'en'
    ? (priceIdx >= 1.3 ? 'Premium 💎' : priceIdx >= 1.05 ? 'Above Average 📊' : priceIdx >= 0.95 ? 'Market Average ⚖️' : 'Budget 🏷️')
    : (priceIdx >= 1.3 ? '프리미엄 💎' : priceIdx >= 1.05 ? '표준 이상 📊' : priceIdx >= 0.95 ? '시장 표준 ⚖️' : '저가 할인 🏷️');
  const priceWarn  = (priceIdx > avgCompPrice * 1.4) || (priceIdx < avgCompPrice * 0.6);
  const priceSectionTitle = LANG.current === 'en' ? 'Price Positioning' : '가격 포지셔닝';
  items.push({
    icon: '🏷️', title: priceSectionTitle, warn: priceWarn,
    body: LANG.current === 'en'
      ? `Price index <strong>${priceIdx.toFixed(2)}</strong> (${priceLabel}) / Competitor avg <strong>${avgCompPrice.toFixed(2)}</strong>`
        + (priceWarn ? `<br><span style="color:var(--amber)">Large gap vs. competitors. Watch for customer churn.</span>` : '')
      : `현재 가격 지수 <strong>${priceIdx.toFixed(2)}</strong> (${priceLabel}) / 경쟁사 평균 <strong>${avgCompPrice.toFixed(2)}</strong>`
  ...
  const cashSectionTitle = LANG.current === 'en' ? '🚨 Cash Runway Warning' : '현금 런웨이 경고';
  if (burnRate > 0 && G.cash / burnRate < 4) {
    items.push({
      icon: '🚨', title: cashSectionTitle, warn: true,
      body: LANG.current === 'en'
        ? `Cash <strong>${fmt(G.cash)}</strong> / Burn ~${fmt(burnRate)}/qtr → <strong>${Math.floor(G.cash / burnRate)} quarters</strong> left. Consider raising capital.`
        : `잔여 현금 <strong>${fmt(G.cash)}</strong> / 분기 소진 약 ${fmt(burnRate)} → <strong>${Math.floor(G.cash / burnRate)}분기</strong> 남음. 자금 조달을 검토하세요.`,
    });
  }
  ...
  const overall   = warnCount === 0
    ? { emoji: '✅', msg: LANG.current === 'en' ? 'All indicators look good. Ready to run the quarter.' : '모든 지표가 양호합니다. 분기를 실행해도 좋습니다.' }
    : warnCount === 1
    ? { emoji: '⚠️', msg: LANG.current === 'en' ? 'One item needs attention. Review before running.' : '주의 항목이 있습니다. 확인 후 실행하세요.' }
    : { emoji: '🚨', msg: LANG.current === 'en' ? `${warnCount} warning signals detected. Review carefully.` : `${warnCount}개 항목에서 위험 신호가 감지됩니다. 신중히 검토하세요.` };
  ...
  document.getElementById('preq-date').textContent   = LANG.current === 'en'
    ? `Q${G.quarter} ${G.year} Pre-Quarter Review`
    : `Q${G.quarter} ${G.year} 실행 전 점검`;
...
function getHeadlinesForYear(year) {
  const db = (typeof LANG !== 'undefined' && LANG.current === 'en' && typeof HEADLINES_EN !== 'undefined')
    ? HEADLINES_EN : HEADLINES;
  const fallback = (typeof LANG !== 'undefined' && LANG.current === 'en')
    ? ['Quarter in progress…'] : ['분기가 진행 중입니다...'];
  if (typeof db === 'undefined') return fallback;
  const keys = Object.keys(db).map(Number).sort((a, b) => a - b);
  const pool = [];
  keys.filter(k => Math.abs(k - year) <= 3).forEach(k => pool.push(...db[k]));
  if (pool.length) return pool;
  const nearest = keys.reduce((a, b) => Math.abs(b - year) < Math.abs(a - year) ? b : a);
  return db[nearest] || fallback;
}
...
function _qrunSecretaryComment() {
  ...
  const minEmp = calcMinEmployees();
  const en = typeof LANG !== 'undefined' && LANG.current === 'en';
  if (G.cash < 0) return en ? 'Cash reserves depleted. Emergency funding required.' : '현금이 바닥났습니다. 긴급 자금 조달이 필요합니다.';
  if (G.employees < minEmp * 0.8) return en ? 'Understaffed. Revenue losses incoming without headcount increase.' : '인력이 부족합니다. 충원 없이는 매출 손실이 발생합니다.';
  if (h && h.profit < 0) return en ? 'Cost reduction and profitability improvement are urgent.' : '비용 절감과 수익성 개선이 시급합니다.';
  if (alive <= 1) return en ? 'Competitors nearly gone. Time to consolidate market dominance.' : '경쟁사들이 거의 사라졌습니다. 시장 지배력을 강화하세요.';
  if (alive >= 4) return en ? `${alive} competitors are moving aggressively.` : `${alive}개 경쟁사가 치열하게 움직이고 있습니다.`;
  const c = en
    ? ['Sales team is acquiring new customers.',
       'Dev team is focused on feature improvements.',
       'Closely monitoring competitor movements.',
       'Market data is being aggregated.',
...
function launchQuarterRun() {
  ...
  const _qrEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  document.getElementById('qrun-q').textContent    = `Q${G.quarter}`;
  document.getElementById('qrun-year').textContent  = G.year;
  document.getElementById('qrun-stats').innerHTML   = _qrEn
    ? `<div class="qrun-stat"><div class="qrun-stat-label">Cash</div><div class="qrun-stat-val">${fmt(G.cash)}</div></div>
    <div class="qrun-stat"><div class="qrun-stat-label">Share</div><div class="qrun-stat-val">${relShare}%</div></div>
    <div class="qrun-stat"><div class="qrun-stat-label">Staff</div><div class="qrun-stat-val">${G.employees.toLocaleString()}</div></div>`
    ...
    <div class="qrun-stat"><div class="qrun-stat-label">직원</div><div class="qrun-stat-val">${G.employees.toLocaleString()}명</div></div>`;
...
function _executeTurn() {
  ...
  document.getElementById('hire-cost-txt').textContent = (typeof LANG !== 'undefined' && LANG.current === 'en') ? 'Hiring cost $0' : '채용비 $0';
...
function _fmtSaveSlot(s) {
  ...
  const _fssEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  return {
    name:    s.name    || (_fssEn ? '(untitled)' : '(무제)'),
    product: s.product || '',
    year:    s.year    || '—',
    quarter: s.quarter || '—',
    cash:    s.cash    != null ? fmt(s.cash) : '—',
    rev:     s.revenue != null ? fmt(s.revenue) : '—',
    emp:     s.employees != null ? s.employees + (_fssEn ? '' : '명') : '—',
...
function refreshSaveSlots(containerId, mode) {
  ...
  if (!el) return;
  const _ssEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  el.innerHTML = slots.map(s => {
    ...
    if (!d) {
      return `<div class="save-slot empty" onclick="${mode==='save'?`saveToSlot(${s.slot})`:`loadGame(${s.slot})`}">
        ...
        <span class="ss-empty">${_ssEn ? (mode==='save'?'Empty — save here':'Empty slot') : ('빈 슬롯'+(mode==='save'?' — 여기에 저장':''))}</span>
      </div>`;
    }
    return `<div class="save-slot filled" onclick="${mode==='save'?`saveToSlot(${s.slot})`:`loadGame(${s.slot})`}">
      ...
      <div class="ss-stats">
        <span>💰 ${d.cash}</span>
        <span>📈 ${d.rev}${_ssEn?'/qtr':'/분기'}</span>
        <span>👤 ${d.emp}</span>
      </div>
      ${mode==='save'?`<div class="ss-overwrite">⚠ ${_ssEn?'Overwrite':'덮어쓰기'}</div>`:''}
    </div>`;
  }).join('');
}
...
function saveToSlot(slot) {
  ...
  closeSaveModal();
  const _svEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const b = document.querySelector('.btn-save');
  b.textContent = ok
    ? (_svEn ? `Saved ✓ (Slot ${slot})` : `저장됨 ✓ (슬롯${slot})`)
    : (_svEn ? 'Save Failed ✗' : '저장 실패 ✗');
  setTimeout(() => b.textContent = _svEn ? 'Save' : '저장', 2000);
}
...
function refreshContinueButton() {
  ...
  if (saved && saved.year) {
    ...
    const _cbEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    sub.textContent = `${saved.name || '—'} · Q${saved.quarter} ${saved.year} · ${_cbEn ? 'Cash' : '현금'} ${cash}`;
    btn.style.display = '';
...
function loadGame(slot) {
  ...
  if (!saved) {
    const _lgEn = typeof LANG !== 'undefined' && LANG.current === 'en';
    alert(_lgEn ? `No saved game in slot ${slot}.` : `슬롯 ${slot}에 저장된 게임이 없습니다.`);
    return;
  }
  Object.assign(G, saved);
  G.firedEvents = new Set(saved.firedEvents || []);
...
function confirmMainMenu() {
  ...
  const _mmEn = typeof LANG !== 'undefined' && LANG.current === 'en';
  const _mmMsg = _mmEn
    ? 'Return to main menu?\n(Current game will be auto-saved.)'
